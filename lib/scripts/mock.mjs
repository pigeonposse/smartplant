...
head( 'La consolidación nocturna, sobre el libro que la librería escribe de verdad' )
{
	const nightly = await createPlant( {
		name : 'Nocturna', species : 'Ficus lyrata',
		sensor : { driver : 'mock', dayNight : false },
		ai : { provider : 'mock' },
	} )
	await nightly.read()

	const led = nightly.resolutions
	const episode = ( action, ok, at ) => {
		led.opened( 'soil-dry', { at } )
		led.acted( action, { at : at + 1000 } )
		led.closed( 'soil-dry', { at : at + ( ok ? 6 : 31 ) * HOUR, resolved : ok } )
	}

	let at = Date.now() - 90 * DAY
	for ( let i = 0; i < 4; i++ ) { episode( 'water-200ml', true, at ); at += 5 * DAY }
	for ( let i = 0; i < 3; i++ ) { episode( 'top-up-50ml', i === 0, at ); at += 5 * DAY }
	// Una tarde en la que se hicieron tres cosas a la vez: no atribuye nada.
	for ( let i = 0; i < 3; i++ ) {
		led.opened( 'soil-dry', { at } )
		led.acted( 'water-200ml', { at : at + 1000 } )
		led.acted( 'feed', { at : at + 2000 } )
		led.closed( 'soil-dry', { at : at + 6 * HOUR, resolved : true } )
		at += 5 * DAY
	}

	const night = await nightly.consolidate( { force : true } )

	// Esto devolvía cero hallazgos sobre un libro lleno: agrupaba por un campo
	// singular que el ledger nunca escribe, y todo caía en un único brazo.
	check( 'encuentra el problema resuelto de dos maneras', night.findings.length === 1,
		night.findings[ 0 ]?.why?.slice( 0, 90 ) )
	check( 'y prefiere el que funcionó', night.findings[ 0 ]?.best === 'water-200ml' )
	check( 'aparta las tardes en que se hicieron varias cosas', night.findings[ 0 ]?.ambiguous === 3 )
	check( 'no emite nada', night.emitted === false )
	check( 'y no puede actuar por su cuenta', night.proposals.every( p => !p.actionable ),
		night.proposals[ 0 ]?.why?.slice( 0, 80 ) )

	await nightly.destroy()
}
