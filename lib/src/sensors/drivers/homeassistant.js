...
export class HomeAssistantSensor extends SensorDriver {
	...
	async _entityState( entityId ) {
		...
		// A hung request is worse than a failed one: without a deadline this never
		// settles, `read()` never returns, and the monitoring loop stops forever
		...
		try {
			...
			res = await fetch( `${this.url}/api/states/${encodeURIComponent( entityId )}`, {
				headers : {
			...
			if ( err.name === 'AbortError' ) {
				...
				throw new SensorError( `Home Assistant did not answer for ${entityId} within ${this.timeout}ms.` )

			}
			throw new SensorError( `Cannot reach Home Assistant at ${this.url}: ${err.message}` )
	...
	async read() {
		...
		states.forEach( ( r, i ) => {
			...
			// Keep *why* each one failed. "Entity not found", "token rejected" and
			// "the server stopped answering" need completely different fixes, and
			// collapsing them into one list of names throws that away.
			else failed.push( `${id} (${r.status === 'rejected' ? r.reason?.message ?? 'error' : `state "${r.value}"`})` )
