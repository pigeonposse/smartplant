...
export class ElectrodeSensor extends SensorDriver {
	...
	constructor( config = {} ) {
		...
		this.buffer   = []
		/** Where each push landed in the buffer, and when. */
		this.marks    = []
	...
	push( samples, opts = {} ) {
		...
		// A mark per push, so a sample can later be located in time. Deriving the
		// position from the sample rate alone works only while recording is
		// continuous, and it never is — processes pause, transports stall. The
		// marks make gaps visible instead of silently shifting everything.
		if ( added ) {

			// A driver that runs on its own clock — a simulation, a replayed
			// recording — says when these samples belong. Wall time is only right
			// when the samples really did arrive as they were produced.
			this.marks.push( {
		...
		if ( this.buffer.length > this.capacity ) {
			...
			const dropped = this.buffer.length - this.capacity
			this.buffer.splice( 0, dropped )
			// Marks index into the buffer, so they move with it.
			for ( const m of this.marks ) m.index -= dropped
			this.marks = this.marks.filter( m => m.index >= 0 )
	...
	advance( seconds ) {
		...
		// Simulated seconds, not elapsed ones: the buffer's marks must describe the
		// time the samples represent, or a window "around" a past event would look
		// for it in the wrong place.
		this._simulatedAt = ( this._simulatedAt ?? Date.now() ) + seconds * 1000
		this.push( samples, { at : this._simulatedAt } )
	...
	/**
	 * The driver's own notion of now.
	 *
	 * Real transports run on wall time. A synthetic one runs on the time it has
	 * been asked to simulate, which can be hours ahead of the clock on the wall.
	 *
	 * @returns {number} Milliseconds.
	 */
	now() {

		return this._simulatedAt ?? Date.now()

	}

	/**
	 * A window of samples centred on a past moment.
	 *
	 * Used to characterise what the plant did around something known to have
	 * happened — a watering, a feed — where the care log has the time but the
	 * buffer has only values.
	 *
	 * @param   {number}   at      - When, in ms.
	 * @param   {number}   seconds - How much to take after it.
	 * @param   {object}   [opts]  - `{ leadSeconds }` to include before it too.
	 * @returns {number[]|null}    Samples, or null if that moment is not covered.
	 */
	windowAround( at, seconds, opts = {} ) {

		if ( !this.marks.length || !Number.isFinite( at ) ) return null

		const lead = opts.leadSeconds ?? 0
		const from = at - lead * 1000
		const to   = at + seconds * 1000

		// The buffer is a ring: anything older than the earliest mark is gone.
		if ( from < this.marks[ 0 ].at ) return null
		if ( to > this.marks.at( -1 ).at ) return null

		const indexAt = when => {

			// The nearest mark at or before `when`, then step forward at the sample
			// rate. Interpolating from a *nearby* mark keeps the error bounded by
			// one push rather than by the age of the whole buffer.
			...
			return Math.round( mark.index + ( ( when - mark.at ) / 1000 ) * this.sampleRate )

		}

		const start = Math.max( 0, indexAt( from ) )
		const end   = Math.min( this.buffer.length, indexAt( to ) + 1 )

		return end - start >= 2 ? this.buffer.slice( start, end ) : null
