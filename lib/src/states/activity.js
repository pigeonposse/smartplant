...
export class Activity {
	...
	constructor( opts = {} ) {
		...
		// How long the same line has to stay away before it counts as new.
		this.dedupeMs = opts.dedupeMs ?? 30 * 60_000
		this.entries = []
	...
	add( entry ) {
		...
		if ( finite( last?.at ) && at - last.at < ( this.dedupeMs ?? 30 * 60_000 ) ) {

			last.entry.at = at
			last.at = at
			// The newest detail wins: what the reader wants is the current value,
			// with a count saying how long it has been saying it.
	...
	toJSON() {
		...
		return { entries : this.entries.slice( -60 ) }
