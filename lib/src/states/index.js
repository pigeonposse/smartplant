...
export function stateCues( s ) {
	...
	if ( s.evidence.length ) cues.push( `Because: ${s.evidence.map( e => e.detail ).join( ' ' )}` )

	if ( !s.acts ) {

		cues.push( `Confidence is too low for this to change what the system does — it decides ${s.decides}, and it is not being allowed to. Worth a person looking.` )
