...
export function counterfactual( ledger, problem ) {
	...
	//
	// The ledger records `actions` — an array, because an episode can have
	// several things done during it. Reading a singular `action` off these
	// entries, as this did, found `undefined` every time and dropped the whole
	// history into one arm called "nothing": two arms could never be found, so
	// no counterfactual was ever known and consolidation reported "nothing has
	// been met more than one way" on a ledger that was full of it. A function
	// that worked and was never reached.
	//
	...
	// recovers which one helped — the same rule `whatWorked()` applies, and it
	// has to be the same rule or the two would disagree about the same ledger.
	const arms = new Map()
	let ambiguous = 0
	...
	for ( const entry of history ) {
		...
		const action = actions[ 0 ] ?? 'nothing'
	...
	if ( usable.length < 2 ) {
		...
		return {
			...
			arms : usable,
			ambiguous,
			why : `"${problem}" has only ever been met one way${usable.length ? ` (${usable[ 0 ].action}, ${usable[ 0 ].n} times)` : ''}. A counterfactual needs a time somebody did something else, and there is not one.${ambiguous ? ` ${ambiguous} episode(s) had several things done at once and cannot attribute anything to any of them.` : ''} This is the ordinary answer and it is the correct one — it is not a gap to be filled by imagining the other arm.`,
		}
