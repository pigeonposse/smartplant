...
export function createSource( spec ) {
	...
	// `driver` is what every other layer in this library calls it — sensors and
	// spectral fixtures both. Vision came out as `source`, and the mismatch is
	// the sort of thing that costs somebody twenty minutes: an unrecognised key
	// silently left the default in place, and the error that eventually arrived
	// was about /dev/video0, which they had never mentioned.
	const { source, driver, ...config } = spec || {}
	const kind = source ?? driver ?? 'ffmpeg'
	...
	switch ( kind ) {
		...
		default:
			throw new SmartPlantError( `Unknown vision source "${kind}". Use ffmpeg, raw-file or callback.`, 'VISION_ERROR' )
