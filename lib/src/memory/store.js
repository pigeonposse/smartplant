...
export class PlantMemory {
	...
	constructor( opts = {} ) {
		...
		this.data = {
			...
			events   : [],
			notes    : [],
			// What the system has worked out about itself, as opposed to what it
			// has measured. None of it can be recomputed from the readings — the
			// calibration record costs twelve overrides, which is months — and
			// until now all of it was lost the moment the process ended.
			symbiont : {
				calibration : null,
				trajectory  : null,
				adaptation  : null,
...
export function linearTrend( values ) {
	...
	// Called on a metric nothing has recorded yet, which is normal early on.
...
function migrate( parsed ) {
	...
	// A file written before this existed has no symbiont block, and spreading
	// `parsed` over the defaults would leave it undefined rather than empty.
	data.symbiont = {
		calibration : null,
		trajectory  : null,
		adaptation  : null,
