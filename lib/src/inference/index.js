 ...
 *
 * The distinction matters more than it looks. `missing` is read by the system
 * diagnosis and printed as "add this sensor", so reporting it for an inference
 * that is simply waiting for more readings sends somebody out to buy a probe
 * they already own — and then it still will not work, because the probe was
 * never the problem.
 *
 * So an inference that has the instruments and not the history says exactly
 * that, and names how much more it needs.
 *
 * @param   {string} id     - Inference id.
 * @param   {string} label  - Human label.
 * @param   {string} why    - What is being waited for.
 * @returns {object}        The inference.
 */
const waiting = ( id, label, why ) => ( {
	id,
	label,
	known : false,
	// Deliberately empty: nothing is missing from the rig.
	missing : [],
	waiting : true,
	why,
} )
...
export function uptakeBalance( rows, opts = {} ) {
	...
	// Genuinely absent instruments. This is the only branch that should ever
	// tell somebody to buy something.
	if ( !paired.length ) {

		return needs( 'uptake', 'Water and salt uptake', [ 'soil', 'conductivity' ],
			'This is what separates a plant drinking normally from one concentrating fertiliser around its own roots.' )

	}

	if ( paired.length < 6 ) {

		return waiting( 'uptake', 'Water and salt uptake',
			`Both probes are reporting; there are only ${paired.length} paired readings so far and this needs 6. Nothing to buy — it fills in on its own.` )

	}

	// Take the last six or more paired readings rather than a fixed clock
	// window. A plant read four times a day never accumulates six readings in
	// twelve hours, and an inference that only works on a rig sampling every ten
	// minutes is one that quietly never runs on most of them.
	const window = paired.slice( -Math.max( 6, opts.samples ?? 8 ) )

	const first = window[ 0 ], last = window.at( -1 )
	const spanHours = ( new Date( last.t ).getTime() - new Date( first.t ).getTime() ) / 3_600_000

	// Too long a span crosses a watering, which resets both series and makes the
	// comparison meaningless in a way that looks like a finding.
	if ( !Number.isFinite( spanHours ) || spanHours > ( opts.maxSpanHours ?? 72 ) ) {

		return waiting( 'uptake', 'Water and salt uptake',
			`The last ${window.length} paired readings span ${Math.round( spanHours )}h, which almost certainly crosses a watering. Moisture and conductivity both reset when the pot is watered, so comparing across one produces a confident number about nothing. Reading more often would fix this.` )

	}

	const hours = Math.max( 1, Math.round( spanHours ) )
...
export function lightSaturation( rows ) {
	...
	if ( usable.length < 12 ) {
		...
		return waiting( 'lightSaturation', 'Light saturation',
			`PAR and a response channel are both reporting; there are ${usable.length} usable readings and this needs 12 to separate a saturating curve from noise. Nothing to buy.` )
...
export function thermalInertia( rows ) {
	...
	if ( usable.length < 12 ) {
		...
		return waiting( 'thermalInertia', 'Substrate thermal inertia',
			`The soil thermometer is reporting; there are ${usable.length} paired readings and this needs 12 to see a swing damp out. Nothing to buy.` )
