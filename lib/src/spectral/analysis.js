...
export function readBlueInContext( key, ctx ) {
	...
	if ( ctx?.archetype?.nocturnal ) {
		...
		// Whether it is day is a fact about the plant's surroundings, so the
		// caller supplies it if it can. Falling back to the host's wall clock is
		// a last resort: a machine in another timezone, or a plant under a lamp
		// on its own schedule, would otherwise have this protection silently
		// switch off at the wrong hour.
		const light = ctx?.current?.light
		const hour = Number.isFinite( ctx?.hour ) ? ctx.hour : new Date( ctx?.at ?? Date.now() ).getHours()

		// A light reading outranks any clock, because it is the actual condition
		// this rule is about.
		const daytime = Number.isFinite( light ) ? light > 500 : hour >= 8 && hour <= 18
