...
export class SpectralSystem {
	...
	_measure( bandId, {
		periodSeconds, cycles, startedAt, tests = 1,
	} ) {
		...
		// A probe that is one band of a sweep must be judged against the whole
		// sweep, not on its own — otherwise the sweep finds a response somewhere
		// roughly a quarter of the time on a plant that did nothing.
		const locking = phaseLocking( samples, rate, periodSeconds, { tests } )
	...
	async sweep( opts = {} ) {
		...
		// The atmospheric context is captured once, before the sweep starts, so
		// every band is interpreted against the same conditions.
		...
		for ( const id of bands ) {
			...
			try {
				...
				responses[ id ] = await this.probe( id, {
					...opts,
					// Every band in this sweep is one test of the same question.
					tests : bands.length,
				} )
...
export {
	CHANNELS, fixtureCapability, FIXTURE_IDS, FIXTURES, recipe, RECIPES,
} from './fixtures.js'
