...
export class SpectralSafety {
	...
	constructor( opts = {} ) {
		...
		/** How old the gating readings may be before a treatment is refused. */
		this.maxDataAgeMinutes = opts.maxDataAgeMinutes ?? 30
	 ...
	 */
	/**
	 * How old the data gating a decision is.
	 *
	 * @param   {object} ctx - Plant context.
	 * @returns {object}     `{stale, ageMinutes}`.
	 */
	_staleness( ctx ) {

		const at = ctx?.current?.timestamp || ctx?.at || ctx?.lastReadingAt

		// No timestamp at all is its own kind of unknown, and the contraindications
		// handle a missing reading already. Do not invent an age for it.
		if ( !at ) return {
			stale : false,
			ageMinutes : null,
		}
		...
		return {
			stale : ageMinutes > this.maxDataAgeMinutes,
	...
	validate( request, ctx ) {
		...
		// ── treatment-specific ─────────────────────────────────────────────────
			...
			// 0. Freshness. The contraindications already refuse to act on data that
			// is *absent*, but a reading from three hours ago is worse than absent:
			// it looks like knowledge. Blue forces stomata open, and the whole point
			// of gating it on VPD and soil is to know what the plant is facing right
			// now. Stale numbers are treated as no numbers, which routes them
			// straight into the existing missing-data block.
			const stale = this._staleness( ctx )

			if ( stale.stale ) {

				deny( `The readings gating this treatment are ${stale.ageMinutes} minutes old, and a treatment that forces a physiological response has to be decided on current conditions. Take a reading first.` )

			}

			// 1. Contraindications: the hard blocks.
			for ( const c of contraindications( bandId, stale.stale ? {} : ctx ) ) {
