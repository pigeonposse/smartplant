...
import { readSpace as spatial } from '../spatial/index.js'
...
export function vitals( plant ) {
	...
	// The page somebody opens *because* something is wrong should not need a
	// well-formed plant in order to render.
	plant = plant ?? {}
...
export async function snapshot( plant, opts = {} ) {
	...
	const payload = {
		...
		// Every number in this payload is Celsius. The unit travels beside them
		// as a display instruction and is applied by the page, at the last
		// possible moment — a JSON feed whose numbers change meaning with a
		// setting is one that cannot be diffed, cached or compared between two
		// plants configured differently.
		units : plant.units ?? 'metric',
		...
		layers : {
			...
			lidar : Boolean( plant.sensors?.peek?.( 'lidar' ) ),
			presence : plant.perception?.presence?.sensing ?? Boolean( plant.sensors?.peek?.( 'presence' ) ),
		...
		colony : plant.colony?.enabled
			? {
				...
				// What the plants have actually said to each other. A person can
				// read every exchange and author none of them, which is the whole
				// arrangement — so this is the only place it is visible.
				transcript : ( plant.colony.transcript ?? [] ).slice( -40 ).map( t => ( {
					from : t.from,
					to : t.to,
					kind : t.kind,
					text : t.text,
	...
	// The space, when something can measure it. Not a vital sign — a scan is not
	// a number that goes up and down — but it belongs in the inventory, because
	// "the neighbour is 31cm away, measured" and "somebody typed 25cm" are
	// different facts and the screen should say which one it is showing.
	const lidar = plant.sensors?.peek?.( 'lidar' )
	const scan = lidar?.latest

	payload.space = scan
		? ( () => {

			const read = safely( () => spatial( scan, {
				radius : ( plant.chassis ?? plant ).baseM / 2,
				neighbours : plant.neighbourBearings ?? [],
			} ) )

			return read && {
				returns : read.returns,
				clear : read.clearance?.clear ?? null,
				tightest : read.clearance?.tightest?.metres ?? null,
				openings : read.clearance?.openings?.length ?? 0,
				neighbours : read.neighbours.filter( n => n.measured ).map( n => ( {
					id : n.id,
					metres : n.metres,
				} ) ),
				why : read.why,
		...
		: lidar
			? {
				stale : true,
				why : lidar.freshness().why,
			}
			: null

	// Whether anybody is in the room, when a radio can say. On the panel because
	// it is the difference between a plant nobody has been near for a week and
	// one somebody walks past hourly, and the second explains a lot of readings.
	const presence = plant.perception?.presence

	payload.presence = presence?.settled
		? {
			occupancy : presence.occupancy,
			motion : presence.motion,
			sensing : presence.sensing,
			why : presence.why,
		}
		: presence
			? {
	...
	// What it just did or noticed. Not readings — a feed with every sensor tick
	// in it is a log rather than a story.
	// The whole record, not a window on it. A feed that shows the last thirty
	// lines is a feed that cannot answer what happened this morning.
	payload.activity = safely( () => plant.activity?.recent?.( { limit : 400 } ) ) ?? []

	// What a watering would be, which is the one thing a person with a pump
	// actually needs to see before trusting it.
	payload.watering = safely( () => {

		const plan = plant.wateringPlan?.()
		return plan?.known
			? {
				ml : plan.ml,
				seconds : plan.seconds ?? null,
				litres : plan.litres,
				why : plan.why,
			}
			: {
	...
	// The pot as part of the body: whether it is running out, and whether the
	// plant is still settling into a new one.
	payload.rootSpace = safely( () => {
		...
		return r && {
			...
			daysInPot : r.daysInPot ?? null,
			settling : r.settling === true,
			evidence : ( r.evidence ?? [] ).map( e => e.detail ),
			why : r.why,
		}

	} ) ?? null

	payload.transplant = safely( () => {

		const t = plant.transplantStatus?.()
		return t && t.phase !== 'none' ? t : null

	} ) ?? null

	// The year the plant is living in, and its own clock. Both are slow things
	// that never appeared anywhere a person could see them.
	payload.season = safely( () => plant.seasonalRanges?.() ) ?? null

	payload.rhythms = {
		circadian : safely( () => plant.states?.().circadian_integrity?.level ) ?? null,
		clock : safely( () => plant.perception?.electro?.clock?.periodHours ) ?? null,
	}

	// What survived the last restart, which is the one thing a person cannot
	// work out by looking at the plant.
	payload.symbiont = {
		restored : ( plant._rehydrated?.restored ?? [] ).join( ', ' ) || null,
		at : plant.memory?.data?.symbiont?.at ?? null,
	}
