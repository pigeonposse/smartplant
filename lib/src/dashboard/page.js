...
function report( d ) {
	const sig = JSON.stringify( [ d.vitals.map( v => [ v.metric, v.measured, v.band ] ), d.layers,
		d.presence && [ d.presence.occupancy, d.presence.motion ], d.space && [ d.space.clear, d.space.tightest ] ] )
	...
	if ( d.presence ) {
		html += '<div class="grp">the room</div>'
		html += d.presence.settling
			? '<div class="item off"><span class="s none">·</span><span class="k">presence</span>' +
				'<span class="v">settling</span></div>'
			: '<div class="item"><span class="s ' + ( d.presence.occupancy ? 'high' : 'ok' ) + '">' +
				( d.presence.occupancy ? '●' : '·' ) + '</span>' +
				'<span class="k">' + ( d.presence.motion ? 'somebody moving' : d.presence.occupancy ? 'somebody here' : 'empty' ) +
				'</span><span class="v">' + esc( d.presence.sensing ) + '</span></div>'
	}

	if ( d.space ) {
		html += '<div class="grp">space</div>'
		if ( d.space.stale ) {
			html += '<div class="item off"><span class="s none">·</span><span class="k">last scan</span>' +
				'<span class="v">too old</span></div>'
		} else {
			html += '<div class="item"><span class="s ' + ( d.space.clear ? 'ok' : 'high' ) + '">' +
				( d.space.clear ? '●' : '▲' ) + '</span><span class="k">nearest</span>' +
				'<span class="v">' + d.space.tightest + 'm</span></div>'
			html += '<div class="item"><span class="s ok">·</span><span class="k">ways out</span>' +
				'<span class="v">' + d.space.openings + '</span></div>'
			html += d.space.neighbours.map( n =>
				'<div class="item"><span class="s ok">●</span><span class="k">' + esc( n.id ) + '</span>' +
				'<span class="v">' + n.metres + 'm</span></div>' ).join( '' )
		}
	}
