...
#page-dashboard { overflow:hidden; display:none; grid-template-rows:auto auto 1fr auto auto;
...
/* ── console ────────────────────────────────────────────────────────────── */
/* Newest at the top and growing upward. The last thing that happened is the
   thing being looked for, and it should not be at the bottom of a list that
   has to be scrolled to reach it. */
...
/* ── chat ───────────────────────────────────────────────────────────────── */
.latest { display:flex; align-items:baseline; gap:.7rem; width:100%; text-align:left;
	background:var(--panel); border:1px solid var(--line); border-radius:.55rem;
	padding:.6rem .9rem; font:inherit; font-size:.9rem; color:var(--fg); cursor:pointer }
...
.tag.system{color:var(--none)} .tag.refusal{color:var(--high)}
...
function onData( d ) {
	...
	$( 'activity' ).innerHTML = acts.length
		? acts.map( a => '<div class="act' + ( a.degraded ? ' degraded' : '' ) + '">' +
		...
		: '<div class="row off"><span class="k">Nothing worth mentioning yet. ' +
			'Readings are not activity.</span></div>'
