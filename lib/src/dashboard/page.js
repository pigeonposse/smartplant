...
#page-dashboard { overflow:hidden; display:none; grid-template-rows:auto auto 1fr;
...
/* Four across, filling the width they have. There was a lot of empty page
   around a narrow column of small cards. */
.grid { display:grid; grid-template-columns:repeat(4,1fr); gap:.9rem;
	margin:0 auto; width:100%; align-content:start; align-items:start }
@media (max-width:60rem) { .grid { grid-template-columns:repeat(2,1fr) } }
@media (max-width:34rem) { .grid { grid-template-columns:1fr } }
.card { background:var(--panel); border:1px solid var(--line); border-radius:.7rem; padding:1.1rem 1.3rem }
.card .k { font-size:.85rem; color:var(--dim) }
.card .v { font:600 2.1rem/1.15 var(--mono); font-variant-numeric:tabular-nums; margin-top:.25rem }
.card .u { font-size:.9rem; color:var(--dim); font-weight:400 }
.card.off .v { color:var(--none) }
.card .note { font-size:.78rem; color:var(--dim); margin-top:.5rem; line-height:1.45 }
.bar { height:5px; background:var(--line); border-radius:3px; margin-top:.7rem; position:relative }
...
.latest { display:flex; align-items:baseline; gap:.7rem; text-align:left;
	...
	padding:.6rem 1rem; font:inherit; font-size:.9rem; color:var(--fg); cursor:pointer;
...
const UNITS = { temperature:'°C', humidity:'%', soil:'%', light:'lux', co2:'ppm', airflow:'m/s',
	...
	redFarRed:'', canopySpread:'°C', soilTemperature:'°C', dewPoint:'°C' }

/* Every temperature in the payload is Celsius; the unit arrives beside it and
   is applied here, last. canopySpread is in the list on purpose and is
   converted differently: it is a difference between two temperatures, so it
   scales by 9/5 and takes no +32 offset. Nine degrees of spread is sixteen
   Fahrenheit degrees, not forty-eight, and a screen that said forty-eight
   would be describing a plant that is not there. */
let imperial = false
const isTemp = m => UNITS[ m ] === '°C'
const isDelta = m => m === 'canopySpread'
const tval = ( n, metric ) => {
	if ( !imperial || n === null || n === undefined || !isTemp( metric ) ) return n
	const out = isDelta( metric ) ? n * 9 / 5 : n * 9 / 5 + 32
	return Math.round( out * 10 ) / 10
}
const tunit = metric => isTemp( metric ) && imperial ? '°F' : ( UNITS[ metric ] ?? '' )
...
function onData( d ) {
	...
	$( 'vitals' ).innerHTML = d.vitals.map( v => {
		...
		return '<div class="card"><div class="k">' + esc( v.metric ) + '</div>' +
			'<div class="v">' + tval( v.value, v.metric ) + ' <span class="u">' + tunit( v.metric ) + '</span></div>' +
			( pct === null ? '' : '<div class="bar ' + v.band + '"><i style="left:' +
				Math.min( 97, Math.max( 0, pct ) ) + '%"></i></div>' ) +
			/* The band is converted as two temperatures rather than as a width:
			   scaling a 10-degree Celsius band by 9/5 without the offset gives a
			   range that looks plausible and is not. */
			( v.range ? '<p class="note">comfortable ' + tval( v.range.min, v.metric ) +
				'–' + tval( v.range.max, v.metric ) + '</p>' : '' ) + '</div>'
	} ).join( '' )
	...
	$( 'latest' ).innerHTML = top
		? '<span class="tag ' + top.kind + '">' + esc( top.kind ) + '</span>' +
			...
			'<span class="more">' + clock( new Date( top.lastAt ?? top.at ) ).slice( 0, 5 ) +
			'  ·  full record →</span>'
	...
	$( 'activity' ).innerHTML = merged.length
		? merged.map( a => '<div class="act' + ( a.degraded ? ' degraded' : '' ) +
			...
			// Repeats are folded into the minute they happened in, so the count is
			// "how many times within this minute" and needs no second clock. A
			// line that still says "still at" an hour later is the bug this
			// replaced: it made a frozen feed look like a live one.
			( a.repeated > 1 ? ' <span class="d">×' + a.repeated + '</span>' : '' ) +
			'</span></div>' ).join( '' )
...
function connect() {
	...
	src.onerror = () => {
		...
		pulse.unshift( {
			at : Date.now(),
			text : 'connection lost — nothing below this line is current',
			lost : true,
		} )
