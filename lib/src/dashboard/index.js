...
export async function serveVitals( plant, opts = {} ) {
	...
	// Filled in after listening, because port 0 means "whatever is free" and the
	// number is not known until the OS has picked one. Reporting the requested
	// port would print an address nothing is listening on.
	const net = {
		host,
		port,
		address : null,
	...
	// The truth about where this is reachable. The LAN address is what somebody
	// would actually type from another device, and it is shown only when the
	// server is listening widely enough for that to be true.
	net.port = server.address()?.port ?? port
	net.lan = host === '127.0.0.1' || host === 'localhost' ? null : lanAddress()
	net.address = net.lan ? `${net.lan}:${net.port}` : `${host}:${net.port}`

	return {
		url : `http://${host === '0.0.0.0' ? 'localhost' : host}:${net.port}`,
		host,
		port : net.port,
		net,
		warnings,
