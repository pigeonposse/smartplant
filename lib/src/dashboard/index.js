...
export async function serveVitals( plant, opts = {} ) {
	...
	const server = createServer( async ( req, res ) => {
		...
		if ( url.pathname === '/vitals.json' ) {
			...
			return json( res, 200, await snapshot( plant, { deep : url.searchParams.has( 'deep' ) } ) )
		...
		if ( url.pathname === '/live' ) {
			...
			res.write( `data: ${JSON.stringify( await snapshot( plant ) )}\n\n` )
			return
		...
		if ( url.pathname === '/' ) {
			...
			return res.end( page( plant.memory?.plant?.name ?? 'plant', everyMs ) )
	...
	const timer = setInterval( async () => {
		...
		const data = `data: ${JSON.stringify( await snapshot( plant ) )}\n\n`
