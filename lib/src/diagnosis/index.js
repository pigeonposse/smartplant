...
import { plausibleTemperature } from '../units/index.js'
...
export async function systemDiagnosis( plant, opts = {} ) {
	...
	await checkContinuityOfSelf( plant, checks )
	await checkSeason( plant, checks )
	await checkPot( plant, checks )
	await checkRootSpace( plant, checks )
	await checkTransplant( plant, checks )
	await checkThermal( plant, checks )
	await checkActivity( plant, checks )
	await checkConsolidation( plant, checks )
	await checkProvenance( plant, checks )
	await checkProfile( plant, checks )
	await checkUnits( plant, checks )
...
/**
 * Whether what this plant works out about itself survives a restart.
 */
async function checkContinuityOfSelf( plant, checks ) {

	const persisted = Boolean( plant.memory?.path )
	const block = plant.memory?.data?.symbiont

	if ( !persisted ) {

		checks.push( line( 'continuity-of-self', RESULT.WARN,
			'Memory is in-process, so the calibration record, the trajectory and the adaptation log end with the process.',
			'That record costs twelve overrides to build, which is months of somebody disagreeing with the system, and none of it can be recomputed from the readings. Pass { memory: { path } }.' ) )
		return
	...
	if ( !block?.at ) {
		...
		checks.push( line( 'continuity-of-self', RESULT.OK,
			'Persisted, and nothing worked out about itself yet — which is what a new plant looks like. It saves on destroy().' ) )
		return

	}

	const age = Date.now() - block.at

	checks.push( line( 'continuity-of-self', RESULT.OK,
		`Last saved ${Math.round( age / 3600_000 )}h ago${plant._rehydrated?.restored?.length ? `, resumed ${plant._rehydrated.restored.join( ', ' )} at startup` : ''}.` ) )

}

/**
 * Whether the comfortable bands know what time of year it is.
 */
async function checkSeason( plant, checks ) {

	const hemisphere = plant.config?.hemisphere

	if ( !hemisphere ) {

		checks.push( line( 'season', RESULT.WARN,
			'No hemisphere, so the comfortable bands are the same in January as in July.',
			'A third of the daylight in winter and a plant that has slowed to match — watering a houseplant in January on a July schedule is the commonest way one is killed. Pass { hemisphere: "north" | "south" | "tropical" }. It is not guessed from a clock: half the guesses would be exactly six months wrong.' ) )
		return

	}

	const seasonal = plant.seasonalRanges?.()

	checks.push( line( 'season', RESULT.OK,
		seasonal?.applied
			? `${seasonal.season}, applied at ${Math.round( seasonal.weight * 100 )}% — the rest comes from this plant's own record.`
			: seasonal?.why ?? `Hemisphere set to ${hemisphere}.` ) )

}

/**
 * Whether anything here knows how big this plant's pot is.
 *
 * The number that turns every watering opinion from a fraction into a volume,
 * and the one nothing can work out for itself.
 */
async function checkPot( plant, checks ) {

	const pot = plant.config?.pot

	if ( !pot?.diameterCm && !pot?.litres ) {

		checks.push( line( 'pot', RESULT.WARN,
			'No pot size, so nothing here can say how much water this plant wants.',
			'The archetype decides what fraction of the substrate to wet; the pot decides the volume, and the same plant in a 12cm pot and a 40cm one wants thirty times the water. Pass { pot: { diameterCm } } or { pot: { litres } } — a tape measure round the rim is enough, and no default is assumed because it would carry that same factor.' ) )
		return
	...
	if ( !plan?.known ) {
		...
		checks.push( line( 'pot', RESULT.WARN, plan?.why ?? 'Pot known, and no watering plan could be made.',
			'Usually the archetype: without one there is nothing to say whether this is a cactus or a fern, and they want opposite things from the same pot.' ) )
		return

	}

	checks.push( line( 'pot', plan.seconds ? RESULT.OK : RESULT.WARN,
		`${plan.litres} litre pot, ${plan.ml} ml a watering${plan.seconds ? `, ${plan.seconds}s of pump` : ''}.`,
		plan.seconds ? null : 'The volume is known and the duration is not: flow rate depends on this pump, this tube and this head height and drifts as the tube ages. Measure it with a jug and a stopwatch and pass { pump: { mlPerSecond } }.' ) )

}

/**
 * Whether this pot is running out of room.
 */
async function checkRootSpace( plant, checks ) {

	const t = plant.transplantStatus?.()

	if ( t && t.phase === 'settling' ) {

		checks.push( line( 'root-space', RESULT.OK,
			`Settling into a ${t.volumeL}L pot, day ${t.day}. ${t.baselines === 'learning' ? 'Learning what this container does.' : ''}`.trim() ) )
		return
	...
	if ( !r || r.index === 'unknown' ) {

		checks.push( line( 'root-space', RESULT.ABSENT,
			r?.why ?? 'No pot size, so nothing here can say whether it is running out.',
			r?.missing?.includes( 'pot.since' )
				? 'Pass `{ pot: { since } }` — the date the plant went into this container. Without it there is a size and no way to measure change against it.'
				: null ) )
		return
	...
	if ( r.index === 'low' ) {
		...
		checks.push( line( 'root-space', RESULT.WARN, r.why,
			`Nothing here has seen a root — this is how the plant behaves in this container, not a picture of the substrate. It is worth tipping it out and looking. When you repot, \`plant.transplant({ volumeL })\` is the only thing to tell the system.` ) )
		return

	}

	checks.push( line( 'root-space', RESULT.OK,
		`${r.index} · ${r.outlook} · ${r.daysInPot} days in this pot.` ) )

}

/**
 * A move into a new pot, and everything it suppresses while it settles.
 *
 * Worth its own line rather than a footnote on root-space, because a settling
 * plant is one where several other lines are deliberately quieter — and a person
 * reading "root-space: not applicable" without knowing why would take it for a
 * missing sensor.
 */
async function checkTransplant( plant, checks ) {

	const t = plant.transplantStatus?.()

	if ( !t || t.phase === 'none' ) {

		checks.push( line( 'transplant', RESULT.ABSENT, 'This plant has not been moved.' ) )
		return
	...
	if ( t.phase === 'settled' ) {
		...
		checks.push( line( 'transplant', RESULT.OK,
			`Settled into ${t.volumeL}L, new baselines adopted.` ) )
		return

	}

	// Not a fault: it is the system being careful on purpose, and the useful
	// thing to say is which conclusions are currently being held back.
	checks.push( line( 'transplant', RESULT.WARN,
		`Settling into ${t.volumeL}L, day ${t.day}. ${t.why}`,
		'Nothing to fix — the caution is deliberate. While this is open, the root-space estimate stands aside, watering leans conservative and the internal states are believed less, because a plant with disturbed roots behaves oddly for reasons that have nothing to do with how it is being looked after. It closes when the readings stop lurching rather than on a date; `plant.settleTransplant()` closes it by hand if you can see it is fine.' ) )

}

/**
 * The thermal camera, which is a different instrument from the vision one.
 */
async function checkThermal( plant, checks ) {

	const cam = plant.sensors?.peek?.( 'thermal' )

	if ( !cam ) {

		checks.push( line( 'thermal', RESULT.ABSENT,
			'No thermal camera. Canopy temperature is unmeasured, so water stress is inferred from the soil and the air rather than seen.' ) )
		return
	...
	if ( !cam.latest ) {
		...
		checks.push( line( 'thermal', RESULT.FAIL,
			'A thermal camera is attached and has produced no frame.',
			'Check the `frame` callback returns { width, height, data } with one temperature per pixel. Nothing else here can be judged until it does.' ) )
		return

	}

	const air = plant.memory?.lastReading?.temperature

	if ( !Number.isFinite( air ) ) {

		checks.push( line( 'thermal', RESULT.WARN,
			'Thermal frames are arriving, and there is no air temperature to compare them against.',
			'The stress index is leaf minus air, and the air half has to come from an ordinary thermometer. Taking it off the background of the same frame would cancel the camera\'s absolute error out of both numbers and make their difference look perfect while meaning nothing. Until then only canopy evenness is available — which is still the finding one leaf clip cannot give you.' ) )
		return
	...
	try {
		...
		const { stressIndex, evenness } = await import( '../vision/thermal.js' )
		const s = stressIndex( cam.latest, air )
		const e = evenness( cam.latest )

		if ( !s.known ) {

			// The interesting failure: a plant that has stopped transpiring is at
			// room temperature and disappears into the wall behind it.
			checks.push( line( 'thermal', RESULT.WARN, s.why,
				'Point the camera at the plant and check it fills a reasonable part of the frame. If it does, this is a finding rather than a fault: a canopy that has stopped cooling itself has nothing to separate it from the background.' ) )
			return

		}

		checks.push( line( 'thermal', e.even === false ? RESULT.WARN : RESULT.OK,
			`Canopy ${s.index <= 0.3 ? 'transpiring freely' : s.index >= 0.7 ? 'largely closed up' : 'part way'} (index ${s.index}, ${s.gap}°C against air)${e.even === false ? `, and uneven by ${e.spread}°C` : ''}.`,
			e.even === false
				? 'Part of this plant is transpiring and part is not, which a contact probe on one leaf would have missed entirely. A blocked vessel, a root problem on one side, or a branch sitting in a draught all look like this before they look like anything visible — worth walking over and comparing the two sides.'
				: null ) )

	}
	catch ( err ) {

		checks.push( line( 'thermal', RESULT.FAIL, `Thermal analysis failed: ${err.message}`,
			'This is a bug rather than a wiring problem: the analysis is pure arithmetic over the frame.' ) )
...
/**
 * The feed — whether the plant's own record of what it did is being kept.
 */
async function checkActivity( plant, checks ) {

	const feed = plant.activity

	if ( !feed ) {

		checks.push( line( 'activity', RESULT.ABSENT, 'No activity feed on this object.' ) )
		return
	...
	if ( !report.entries ) {
		...
		// Not a fault on a plant that has just been wired: readings are not
		// activity, and a feed that logged every tick would be a log rather than
		// a record of what happened.
		checks.push( line( 'activity', RESULT.OK, report.why ) )
		return

	}

	const stale = report.latest?.at
		? Date.now() - ( report.latest.lastAt ?? report.latest.at )
		: null

	checks.push( line( 'activity', RESULT.OK,
		`${report.entries} entries across ${Object.keys( report.kinds ).length} kinds, most recently "${report.latest.label}"${Number.isFinite( stale ) && stale > 7 * 86_400_000 ? ` — and nothing for ${Math.round( stale / 86_400_000 )} days` : ''}.` ) )

}

/**
 * Going back over the record, and whether there is a record to go over.
 */
async function checkConsolidation( plant, checks ) {

	if ( typeof plant.consolidate !== 'function' ) {

		checks.push( line( 'consolidation', RESULT.ABSENT, 'Not available on this object.' ) )
		return
	...
	try {
		...
		// Forced, because the diagnosis is being run now rather than at night, and
		// the question here is whether it *would* work rather than whether this is
		// a good moment. Nothing is emitted and nothing acts either way.
		const r = await plant.consolidate( { force : true } )

		if ( !r.findings.length ) {

			checks.push( line( 'consolidation', RESULT.OK, r.why ) )
			return

		}

		const blocked = r.proposals.filter( p => !p.actionable )

		checks.push( line( 'consolidation', RESULT.OK,
			`${r.findings.length} problems have been met more than one way${r.proposals.length ? `, ${r.proposals.length} with a clear difference` : ''}.`,
			blocked.length
				? 'These are proposals rather than changes, and they stay proposals: nothing has ever told this system it was wrong, so a preference formed by rereading its own record is a preference formed from its own conclusions. Overriding a refusal a dozen times is what unlocks acting on them.'
				: null ) )

	}
	catch ( err ) {

		checks.push( line( 'consolidation', RESULT.FAIL, `Consolidation threw: ${err.message}`,
			'A bug rather than a missing sensor — it runs entirely on stored data and should not be able to throw.' ) )
...
/**
 * Who said what, and whether two layers are currently contradicting each other.
 */
async function checkProvenance( plant, checks ) {

	const prov = plant.provenance

	if ( !prov ) {

		checks.push( line( 'provenance', RESULT.ABSENT, 'No claim index on this object.' ) )
		return

	}

	const report = prov.report()
	const clashes = prov.contradictions()

	if ( !report.claims ) {

		checks.push( line( 'provenance', RESULT.OK,
			'Nothing has been claimed about this plant yet, so there is nothing to trace or to disagree about.' ) )
		return
	...
	if ( clashes.length ) {
		...
		checks.push( line( 'provenance', RESULT.WARN,
			`${clashes.length} subject(s) where two layers currently say different things: ${clashes.map( c => c.subject ).join( ', ' )}.`,
			`${clashes[ 0 ].why} This is the one thing the index exists for: each layer is right within its own view, which is exactly why the disagreement is invisible from inside either of them. \`plant.provenance.caseFor( "${clashes[ 0 ].subject}" )\` prints both cases with their evidence.` ) )
		return

	}

	checks.push( line( 'provenance', RESULT.OK,
		`${report.claims} claims about ${report.subjects} subjects from ${report.sources.length} layers, none of them currently contradicting.` ) )

}

/**
 * The profiler, which is off unless somebody asked for it.
 */
async function checkProfile( plant, checks ) {

	const report = plant.profile?.report?.()

	if ( !report ) {

		checks.push( line( 'profile', RESULT.ABSENT, 'No profiler on this object.' ) )
		return
	...
	if ( !report.enabled ) {
		...
		checks.push( line( 'profile', RESULT.ABSENT, report.why ) )
		return

	}

	checks.push( line( 'profile', report.slow?.length ? RESULT.WARN : RESULT.OK, report.why,
		report.slow?.length
			? 'Worth looking at means finding out why it is slow, not wrapping it in a faster copy that then has to agree with the original forever.'
			: null ) )

}

/**
 * Which unit a person is shown, and whether a probe is lying about its own.
 *
 * The display half is trivial. The half worth a line in a diagnosis is the
 * other one: a Fahrenheit probe declared as Celsius reports a room at 72, which
 * passes as a number and is a plant on fire.
 */
async function checkUnits( plant, checks ) {

	const units = plant.units ?? 'metric'
	const declared = plant._drivers?.filter( d => d?.unit ) ?? []

	const temps = [
		plant.memory?.lastReading?.temperature,
		plant.memory?.lastReading?.soilTemperature,
	].filter( Number.isFinite )

	const bad = temps.map( t => plausibleTemperature( t ) ).find( r => !r.ok )

	if ( bad ) {

		checks.push( line( 'units', RESULT.FAIL, bad.why,
			bad.suspect === 'F'
				? 'Declare it at the wire: `{ sensor: { driver, unit: "F" } }`. The conversion belongs there, once, so every layer past it can assume Celsius — and this is deliberately not rescaled here, because a number quietly corrected in the display would hide the mislabelling for the life of the plant while every threshold below carried on reading the wrong one.'
				: 'Check the probe is connected and in the soil rather than dangling.' ) )
		return

	}

	checks.push( line( 'units', RESULT.OK,
		`Showing ${units === 'imperial' ? 'Fahrenheit' : 'Celsius'}${declared.length ? `, ${declared.length} driver(s) declaring their own unit` : ''}. Everything underneath is Celsius.` ) )
...
/**
 * What each installed plugin needs, and whether it has it.
 *
 * A plugin whose hardware is missing is not broken — it is a plugin waiting for
 * a wire, and it will answer every call with a refusal until it gets one. That
 * is a very quiet way for somebody to conclude the library does not work, so it
 * is named here rather than left to be discovered one empty result at a time.
 */
const PLUGIN_NEEDS = {
	transplant : {
		has : p => Boolean( p.config?.pot?.litres || p.config?.pot?.diameterCm ),
		want : 'a pot size — `{ pot: { litres, since } }`',
	},
	thermal : {
		has : p => Boolean( p.sensors?.peek?.( 'thermal' ) ),
		want : 'a thermal camera on the `thermal` driver',
	},
	presence : {
		has : p => Boolean( p.sensors?.peek?.( 'presence' ) ),
		want : 'a WiFi radio on the `presence` driver',
	},
	season : {
		has : p => Boolean( p.config?.hemisphere ),
		want : 'the hemisphere — `{ hemisphere: "north" | "south" | "tropical" }`, which is never guessed',
	},
	energy : {
		has : p => Boolean( p.config?.power?.capacityWh ),
		want : 'a battery — `{ power: { capacityWh } }`. Without one this is a mains plant and nothing is being budgeted',
	},
	spectrum : {
		has : p => Boolean( p.spectral ),
		want : 'a spectral probe — `plant.useSpectral( … )`',
	},
	colony : {
		has : p => Boolean( p.colony?.enabled ),
		want : 'a transport — `use( colony, { transport } )`',
	},
}
...
async function checkPlugins( plant, checks ) {
	...
	// An installed plugin whose documented method has gone missing is a real
	// failure, and the one nothing else in this file would catch.
	const broken = names.filter( n => {

		const api = plant.plugin( n )
		return !api || typeof api.init !== 'function'

	} )

	if ( broken.length ) {

		checks.push( line( 'plugins', RESULT.FAIL,
			`${broken.join( ', ' )} installed but not usable.`,
			'A plugin has to be built with `definePlugin`, or expose `init( plant )` itself. Anything else installs and then does nothing, which looks exactly like the library ignoring it.' ) )
		return

	}

	const waiting = names
		.filter( n => PLUGIN_NEEDS[ n ] && !safely( () => PLUGIN_NEEDS[ n ].has( plant ) ) )
		.map( n => `${n} wants ${PLUGIN_NEEDS[ n ].want}` )

	checks.push( line( 'plugins', waiting.length ? RESULT.WARN : RESULT.OK,
		`${names.length} installed: ${names.join( ', ' )}.${waiting.length ? ` ${waiting.length} of them cannot do anything yet.` : ''}`,
		waiting.length
			? `Each of these will keep answering with a refusal until it has what it needs, which is the honest behaviour and also a quiet way to conclude nothing works: ${waiting.join( '; ' )}.`
			: null ) )
