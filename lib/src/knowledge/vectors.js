 ...
 */
async function withDeadline( url, init = {}, ms = 20000 ) {
	...
	try {
		...
		if ( err.name === 'AbortError' ) throw new Error( `The embedding endpoint did not answer within ${ms}ms.` )
		throw err
...
export class ApiEmbedder {
	...
	async embed( text ) {
		...
		const res = await withDeadline( this.url, {
			method  : 'POST',
			headers : {
...
export class RemoteVectorStore {
	...
	async _request( path, options = {} ) {
		...
		const res = await withDeadline( `${this.url}${path}`, {
