...
import { temperatureSymbol as unitSymbol, temperatureValue as unitValue } from '../units/index.js'
 ...
 * @param   {string} [units] - `'metric'` or `'imperial'`. Display only.
 * @returns {string}         Rendered line.
 */
export function statusLine( ctx, units ) {

	if ( !ctx?.current ) return '🌱 no readings yet'
	...
	for ( const [ metric, meta ] of Object.entries( METRICS ) ) {
		...
		// The emoji is still chosen on the Celsius value against the Celsius
		// band. Only the printed number and its suffix change — comparing a
		// Fahrenheit number against a Celsius range would put every plant in
		// the imperial world permanently on fire.
		const shown = meta.unit === '°C'
			? `${unitValue( value, units )}${unitSymbol( units )}`
			: `${value}${meta.unit}`

		parts.push( `${meta.label}: ${metricEmoji( metric, value, ctx.ranges?.[ metric ] )} ${shown}` )
...
export function offlineVoice( ctx ) {
	...
	// The library must always be able to say *something* true, and "I have no
	// context" is true.
	if ( !ctx ) return 'No context yet — take a reading first.'
