...
export function importBundle( plant, bundle, opts = {} ) {
	...
	// A lesson that carries nothing biological has no species to mismatch. A fern
	// telling a pothos what the room does is describing the room, and the room is
	// the same for both of them.
	const biological = Object.keys( bundle.ranges || {} ).length
		|| ( bundle.policies || [] ).length
		|| bundle.careCadence
		|| bundle.rhythm

	if ( biological && target !== bundle.manifest.species && !opts.force ) {
