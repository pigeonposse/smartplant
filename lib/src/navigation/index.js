...
export async function planMove( plant, move = {}, deps = {} ) {
	...
	// ── is there room to leave at all ───────────────────────────────────────
	// A scan is the only thing here that can answer this. Without one the other
	// checks still run and the plant simply does not know whether it is boxed in.
	const scan = move.scan ?? plant.sensors?.peek?.( 'lidar' )?.latest

	if ( scan ) {

		const { clearance } = await import( '../spatial/index.js' )
		const room = clearance( scan, { radius : ( plant.chassis ?? plant ).baseM / 2 } )

		if ( room.clear === false ) note( REFUSAL.BLOCKED, room.why )
