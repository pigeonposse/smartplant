...
export class AIService {
	...
	constructor( opts = {} ) {
		...
		/** Deadline for one model call. Without it, a hung provider hangs the plant. */
	 ...
	 */
	/**
	 * A deadline for one call, honouring any signal the caller supplied.
	 *
	 * Without this there is no time limit on a model call at all: a provider that
	 * accepts the connection and then stops talking leaves `speak()` pending
	 * forever, and inside the monitoring loop that is a plant that silently stops
	 * being monitored. Built by hand rather than with `AbortSignal.any`, which
	 * needs a newer Node than this library asks for.
	 *
	 * @param   {AbortSignal} [caller] - The caller's signal, if any.
	 * @returns {object}               `{signal, done}`.
	 */
	_deadline( caller ) {
	...
	async generate( prompt, opts = {} ) {
		...
		for ( let attempt = 0; attempt <= this.retries; attempt++ ) {
			...
			// A fresh deadline per attempt: a retry that inherits the exhausted
			// deadline of the attempt before it is not a retry.
			const deadline = this._deadline( opts.signal )
			...
			try {
				...
				lastErr = err.name === 'AbortError' && !opts.signal?.aborted
					? new AIError( `Provider "${this.provider}" did not respond within ${this.timeout}ms.`, { provider : this.provider } )
					: err
				this.stats.failures++
				if ( attempt === this.retries || !isRetryable( lastErr ) ) break
