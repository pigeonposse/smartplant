...
export function uvbGate( plant, rel = {}, opts = {} ) {
	...
	// Stated by the caller, or measured. Until there was a presence sensor this
	// interlock could only be told; now it can be answered.
	const occupied = opts.occupied ?? plant?.perception?.presence?.occupancy
	...
	if ( occupied === true ) {
		...
		return {
			allowed : false,
			reason : 'occupied',
			measured : opts.occupied === undefined,
			why : `Somebody is in the room${opts.occupied === undefined ? ', according to the presence sensor' : ''}. UV-B burns skin and eyes, a plant on a shelf is at eye height, and nothing about a possible aphid justifies that. It waits until the room is empty.`,
		}
