...
export function manifest( plant ) {
	...
	// A lidar or a time-of-flight sensor both answer "how far is that", which is
	// what a neighbour wants to know about this one. `rangefinder` covers a
	// device set directly; the driver covers one attached the usual way, and
	// checking only the first meant an attached lidar advertised nothing.
	add( FACULTY.RANGE, Boolean( plant.rangefinder || plant.sensors?.peek?.( 'lidar' ) ) )
	add( FACULTY.PRESENCE, Boolean( plant.sensors?.peek?.( 'presence' ) ) )
