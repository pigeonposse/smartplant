...
export class ColonyMember {
	...
	async coupling( peer = {}, reference = null ) {
		...
		// If there is a scan and somebody said which way this neighbour is, the
		// distance stops being a number a person typed. That footnote is on every
		// conclusion this layer draws.
		let measuredMetres = peer.measuredMetres

		const scan = this.plant.sensors?.peek?.( 'lidar' )?.latest

		if ( scan && Number.isFinite( peer.bearing ) && !Number.isFinite( measuredMetres ) ) {

			const { rangeTo } = await import( '../spatial/index.js' )
			const ranged = rangeTo( scan, peer )
			if ( ranged.measured ) measuredMetres = ranged.metres
...
export {
	corroborate, CORROBORATION, ENVIRONMENTAL, oddOneOut, SATURATES_AT, worthTeaching,
} from './corroborate.js'
