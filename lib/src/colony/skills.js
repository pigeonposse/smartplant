...
define( {
	id       : 'sense.electrome-fingerprint',
	family   : 'sense',
	says     : 'This is the shape of my electrical baseline right now.',
	requires : [ 'electrode' ],
...
define( {
	id : 'consensus.what-worked', family : 'consensus',
	says : 'This is what actually resolved that problem for me, and how often it cleared on its own.',
	requires : [],
	answer : ( plant, _ctx, payload ) => {

		const ledger = plant._resolutions
		if ( !ledger || !payload?.problem ) return { known : false }

		const worked = ledger.whatWorked( payload.problem, { like : payload.like } )

		return {
			species : plant.memory.plant.species,
			problem : payload.problem,
			known   : worked.known,
			// The base rate travels with the finding, always. "Watering worked
			// 100% of the time" is not information without "and it cleared on its
			// own 90% of the time here" next to it.
			base    : worked.base ?? null,
			actions : worked.actions ?? [],
			episodes: worked.episodes ?? 0,
			why     : worked.why ?? null,
		}
 ...
 * @param   {object} [payload] - Arguments for skills that take them.
 * @returns {object}         `{ok, skill, data}` or `{ok: false, reason}`.
 */
export function answerSkill( plant, skillId, payload ) {
	...
	return {
		...
		skill : skillId,
		says : skill.says,
		data : skill.answer( plant, plant.context(), payload ),
	}
