...
export const DEVICES = {
	...
	'pump-dosing' : d( {
		label     : 'Dosing pump with a measured flow rate',
		layer     : 'actuator',
		provides  : [],
		interface : 'gpio / relay',
		support   : SUPPORT.DRIVER,
		config    : { driver : 'callback', pump : { mlPerSecond : null } },
		notes : 'The difference between this and a bare peristaltic pump is one number: how many millilitres a second it actually moves, through this tube at this height. Measure it with a jug and a stopwatch — it cannot be looked up and it drifts as the tube ages. Without it the system gives you the volume and refuses the duration, which is the honest half of the answer. And it is open-loop: a blocked line, an empty tank and a tube that has fallen out of the pot are identical from the relay, so the soil probe is what tells you the water arrived.',
	} ),
	...
	'thermal-lepton' : d( {
		label     : 'FLIR Lepton / MLX90640 thermal camera',
		layer     : 'plant',
		provides  : [ 'canopySpread' ],
		interface : 'spi / i2c → callback',
		support   : SUPPORT.DRIVER,
		config    : { driver : 'thermal' },
		notes : 'Leaf temperature for every pixel of the canopy, with nothing touching the plant — and one thing a contact probe fundamentally cannot see, which is whether one side of the plant has stopped transpiring while the other has not. Its absolute accuracy is a few degrees, so it does not publish leafTemperature unless you declare it calibrated; what it publishes is the spread across the canopy, which is a difference and survives the sensor offset. The stress index needs an air temperature from a separate thermometer, because taking it off the same frame would cancel the error out of both numbers and make their difference look perfect while meaning nothing.',
	} ),
