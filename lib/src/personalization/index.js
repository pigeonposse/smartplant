 ...
 *
 * ## Two stopping rules, and the second one is not statistical
 *
 * The first is the ordinary one: enough samples, a clear enough effect, stop.
 *
 * The second is a kill switch, and it is the one that matters on a living
 * subject. An experiment that is producing beautiful data on a plant that is
 * declining has not failed as an experiment; it has stopped being one that
 * should be running. So it also watches the plant's internal states, and it
 * abandons the moment they get worse — not when the effect is unclear, but when
 * the subject is.
 *
 * That check runs against the state the plant was in when the experiment
 * *started*, so a plant that was already loaded is not held to a standard it
 * never met, and a plant that deteriorates during the trial is stopped whatever
...
export class Experiment {
	...
	constructor( opts = {} ) {
		...
		/** The plant as it was when this started, so decline is measured from it. */
	...
	/**
	 * Should this stop for the plant's sake rather than for the data's?
	 *
	 * @param   {object} states - Current internal states.
	 * @param   {object} [opts] - `{ wellbeing }`.
	 * @returns {object}        `{stop, why}`.
	 */
	checkSubject( states, opts = {} ) {
		...
		if ( !this.baseline ) {
			...
			return {
				stop : false,
				why : 'No baseline was recorded when this started, so decline cannot be measured against anything. The experiment runs, and its kill switch does not — which is worth knowing before trusting it with a plant.',
			}
		...
		for ( const name of [ 'defense_activation', 'stress_load' ] ) {

			const then = rank[ this.baseline.states?.[ name ]?.level ] ?? 0
			const now = rank[ states?.[ name ]?.level ] ?? 0
			if ( now > then ) worse.push( `${name} ${this.baseline.states?.[ name ]?.level ?? '?'} → ${states[ name ].level}` )

		}

		const dropped = Number.isFinite( opts.wellbeing ) && Number.isFinite( this.baseline.wellbeing )
			&& this.baseline.wellbeing - opts.wellbeing >= 12

		if ( !worse.length && !dropped ) {

			return {
				stop : false,
				why : 'The subject is no worse than when this started.',
			}
		...
		return {
			...
			abandoned : true,
			worse,
			why : `Abandoning. ${worse.length ? worse.join( ', ' ) : `wellbeing fell ${Math.round( this.baseline.wellbeing - opts.wellbeing )} points`} since this began. An experiment producing beautiful data on a plant that is declining has not failed as an experiment — it has stopped being one that should be running, and no result is worth finishing it for.`,
		}
