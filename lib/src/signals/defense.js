...
export function defenseCues( state ) {
	...
	if ( state.evidence.length ) {
		...
		cues.push( `Evidence: ${state.evidence.map( e => e.detail ).join( ' ' )}` )

	}

	if ( state.downgradedFrom ) {

		cues.push( `This was lowered from ${state.downgradedFrom} because the environment accounts for it: ${state.control.why}` )
