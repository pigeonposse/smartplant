...
export function defenseActivation( input = {} ) {
	...
	// like the opening of a wound response. It sits in the same column as the
	// weather, and it is the commoner of the two indoors.
	if ( input.motion?.explains === true && !cause && level !== DEFENSE.LOW ) {

		downgraded = level
		level = level === DEFENSE.HIGH ? DEFENSE.MEDIUM : DEFENSE.LOW
		why = `${input.motion.why} Lowered from ${downgraded} on that basis.`

	}

	if ( !downgraded && control.explains === true && !cause && level !== DEFENSE.LOW ) {
