...
/**
 * Colour and prompts are a nicety, not a requirement.
 *
 * The library itself imports nothing, and a person installing it to talk to a
 * plant from their own code should not be made to carry the CLI's dependencies
 * to do that. So these are optional: if they are present the CLI is pretty, and
 * if they are not it still works in plain text.
 */
...
const real = await optional( 'chalk' )

// Every style falls back to the string itself. `chalk.bold.green(x)` has to keep
// working as a chain, so each stub is a function that is also an object.
const style = () => {

	const passthrough = text => String( text )
	return new Proxy( passthrough, {
		get : ( _t, prop ) => ( prop === 'call' || prop === 'apply' || typeof prop === 'symbol'
			? Reflect.get( passthrough, prop )
			: style() ),
	} )

}

const chalk = real || style()
const enquirer = await optional( 'enquirer' )
...
${chalk.bold( 'Usage' )}
  ...
  smartplant diagnose               Check everything that is wired up, and what to fix
...
${chalk.bold( 'Options' )}
  ...
  --units <id>        metric (°C) | imperial (°F) — display only
...
async function main() {
	...
	switch ( command ) {
		...
		case 'diagnose'     : return cmdDiagnose( flags )
...
async function cmdDiagnose( flags ) {
	...
	const { formatDiagnosis } = await import( './diagnosis/index.js' )
	const plant = await createPlant( configFrom( flags ) )

	console.log( chalk.dim( '\nChecking everything that is connected...' ) )

	// Actually call the model by default: a diagnosis that only checks the key
	// is configured would pass on a provider that is unreachable from here.
	const report = await plant.systemDiagnosis( { probeAI : flags[ 'no-ai-probe' ] !== true } )
	const text = formatDiagnosis( report )

	// Colour only the headings; the check lines carry their own status icon.
	console.log( text
		.replace( 'System diagnosis', chalk.bold( 'System diagnosis' ) )
		.replace( 'What to change', chalk.bold.yellow( 'What to change' ) ) )

	await plant.destroy()
	// A non-zero exit lets this run in a startup script or a cron job and
...
async function interactive( flags ) {
	if ( !enquirer ) {
		...
		console.log( 'The interactive setup needs `enquirer`, which is optional and is not installed.' )
		console.log( 'Install it with `npm i enquirer`, or configure the plant in code instead — see the README.' )
		return
	...
	// Asked rather than inferred from the locale. A machine set to en-US in
	// Vancouver would silently print every temperature in Fahrenheit, and the
	// only symptom would be numbers that still look like plausible temperatures.
	const { units } = await enquirer.prompt( {
		type    : 'select',
		name    : 'units',
		message : 'Temperature',
		choices : [ {
