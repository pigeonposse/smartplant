...
import { motionExplains } from '../sensors/drivers/presence.js'
import { StateCalibration } from '../states/calibration.js'
import { ACTION, permits, posture } from '../states/gate.js'
import { seasonalRanges } from '../archetypes/season.js'
import { rootSpace } from '../archetypes/rootspace.js'
import {
	assess as assessTransplant, describe as describeTransplant, PHASE as TRANSPLANT_PHASE,
	start as startTransplant,
} from '../archetypes/transplant.js'
import { confirm as confirmWatering, dose, pumpSeconds } from '../archetypes/watering.js'
import { Adaptation } from '../states/adaptation.js'
import { Activity } from '../states/activity.js'
import { consolidate } from '../states/consolidation.js'
import { Profile } from '../states/profile.js'
import { Provenance, record as recordProvenance } from '../states/provenance.js'
import { Trajectory } from '../states/trajectory.js'
import {
...
import {
	normalise as normaliseUnits, resolveUnits,
} from '../units/index.js'
...
export class SmartPlant {
	...
	constructor( config = {} ) {
		...
		// A display setting and only a display setting: every threshold, every
		// archetype band and every equation below this line is in Celsius and
		// stays there. Not guessed from a locale — a Canadian laptop set to en-US
		// would flip every temperature on the screen with no other symptom.
		this.units = resolveUnits( this.config )
	...
	async init() {
		...
		// Before anything else reads them: the calibration record, the trajectory
		// and the adaptation log are what this plant has worked out about itself,
		// and none of it is recoverable from the readings.
		this._rehydrated = this.rehydrateSymbiont()
	...
	async read( opts = {} ) {
		...
		results.forEach( ( r, i ) => {
			...
			// A presence radio produces a state rather than a metric, so it does
			// not merge into the reading — but the defence estimate needs to be
			// able to look back at what it said, so it is kept here.
			if ( driver.id === 'presence' && r.status === 'fulfilled' ) this._notePresence( r.value )
		...
		// Unit conversion happens here and nowhere else. A driver that declares
		// `{ unit: 'F' }` — plenty of probes sold in the US do report Fahrenheit —
		// is converted once, at the boundary, so every layer past this line may
		// assume Celsius without checking. Doing it any later means a number that
		// changes unit as it travels, and that mistake is silent: 70 is a
		// comfortable room in Fahrenheit and lethal in Celsius, and both pass
		// every range check in the library.
		const ok      = results
			.map( ( r, i ) => r.status === 'fulfilled'
				? normaliseUnits( r.value, this._drivers[ i ]?.unit )
				: null )
			.filter( Boolean )
	...
	async interrogate( opts = {} ) {
		...
		const may = this.mayI( ACTION.PROBE.id, { force : opts.force } )

		if ( !may.allowed ) {

			return {
				refused : true,
				blockedBy : may.blockedBy,
				why : `Not probing. ${may.why}`,
			}

		}

		// A dampened probe is a shorter one rather than none.
		if ( may.scale < 1 ) opts = {
			...opts,
			scale : may.scale,
		}
	...
	status( reading ) {
		...
		return statusLine( this.context( reading ), this.units )
	...
	async water( detail = {} ) {
		...
		await this.events.emit( 'plant:watered', {
			type : 'water',
	...
	async _ingestLongitudinal( listened ) {
		...
		// One snapshot of what every layer currently believes, so "what is the
		// evidence for this" stops meaning joining five records by hand. Written
		// before the guard below, because the evidence ledger needs an embodied
		// plant and this does not — an index of who claimed what is worth having
		// on a plant that is only being watched.
		added.provenance = recordProvenance( this, this.provenance )
		added.activity = this.activity.notice( this )
	...
	/**
	 * May this plant be asked to do this?
	 *
	 * The one door. Five hand-written gates meant the sixth action somebody added
	 * had none and nothing noticed, because there was no list to be missing from.
	 *
	 * Care — watering, feeding — is never blocked. This withholds elective things
	 * and nothing a plant needs to stay alive.
	 *
	 * @param   {string} action - One of `ACTION`.
	 * @param   {object} [opts] - `{ force }`.
	 * @returns {object}        `{allowed, scale, why}`.
	 */
	mayI( action, opts = {} ) {

		this.calibration ??= new StateCalibration()

		const verdict = permits( this.states(), action, {
			...opts,
			calibration : this.calibration,
		} )

		// A plant whose roots were disturbed a week ago behaves oddly for reasons
		// that have nothing to do with care, and elective things can wait.
		if ( verdict.allowed && !opts.force && this._transplant?.phase === TRANSPLANT_PHASE.SETTLING ) {

			const elective = [ 'probe', 'experiment', 'treat', 'aid', 'move' ].includes( action )
			...
			if ( elective ) {
				...
				verdict.why = `Settling after a transplant. Roots were disturbed and this plant is still finding its rhythm in a new pot — anything elective can wait for that, and a measurement taken now would be measuring the move. Pass { force: true } to override.`

			}

		}

		// An override is the only natural experiment this system gets. Recorded
		// here so the outcome can be graded later, which is the only way a
		// refusal is ever checked.
		if ( verdict.forced ) {

			verdict.trial = this.calibration.overridden( {
				state : verdict.blockedBy[ 0 ].state,
				level : verdict.blockedBy[ 0 ].level,
				confidence : this.states()[ verdict.blockedBy[ 0 ].state ]?.confidence,
				action,
				before : {
					wellbeing : this.happiness(),
					states : this.states(),
				},
			} )
		...
		// A refusal rate is one of the trajectory's axes: a system becoming timid
		// and a plant becoming fragile look the same from inside one decision.
		this._asked = ( this._asked ?? 0 ) + 1

		if ( !verdict.allowed ) {

			this._refused = ( this._refused ?? 0 ) + 1
			// The most useful lines in the feed: somebody looking at a plant that
			// has not been watered wants to know it was declined and why.
			this.activity.refused( action, verdict )

		}
		this._refusalRate = this._refused ? this._refused / this._asked : 0
	 ...
	 *
	 * @param   {string} trial - The handle from a forced `mayI`.
	...
	settleTrial( trial ) {
		...
		return this.calibration.settle( trial, {
			wellbeing : this.happiness(),
			states : this.states(),
		} )
	...
	/**
	 * Move this plant into a different pot.
	 *
	 * The one thing asked for is the new volume, because it is the one thing that
	 * cannot be worked out and the one somebody repotting a plant actually knows.
	 * Everything else — rescaling the doses, closing the old soil baseline,
	 * suspending the root-space estimate, and deciding when it is over — happens
	 * here rather than in a form.
	 *
	 * @param   {object} opts - `{ volumeL, note, electrodeMoved }`.
	 * @returns {Promise<object>} `{started, why}`.
	 */
	async transplant( opts = {} ) {

		const begun = startTransplant( this, opts )

		if ( !begun.started ) return begun

		this._transplant = begun.episode
		this.config.pot = {
			...this.config.pot,
			litres : opts.volumeL,
		...
		// The soil band described a pot that no longer exists. Cleared rather
		// than kept: the probe is somewhere else in a deeper substrate, and the
		// same number means something different.
		if ( this.ranges?.soil ) this.ranges = {
			...this.ranges,
			soil : undefined,
		}
		...
		this.activity?.add?.( {
			kind : 'care',
			label : 'Transplant started',
			detail : `${begun.episode.fromL ? `${begun.episode.fromL}L → ` : ''}${opts.volumeL}L`,
			source : 'user',
		} )

		await this.memory.addEvent( 'transplant', {
			volumeL : opts.volumeL,
			fromL : begun.episode.fromL,
			note : opts.note,
		} ).catch( () => {} )
	...
	/**
	 * Where the transplant is, and whether it is over.
	 *
	 * Decided on the readings rather than the calendar — a plant that settles in
	 * four days should not be treated as fragile for a fortnight.
	 *
	 * @returns {object} `{phase, volumeL, day, why}`.
	 */
	transplantStatus() {
		...
		if ( !this._transplant ) {
			...
			return {
				phase : TRANSPLANT_PHASE.NONE,
				why : 'This plant has not been moved.',
			}

		}

		if ( this._transplant.phase === TRANSPLANT_PHASE.SETTLED ) {

			return {
				...this._transplant,
				why : describeTransplant( this._transplant ),
			}

		}

		const status = assessTransplant( this, this._transplant )

		if ( status.settled ) {

			this._transplant.phase = TRANSPLANT_PHASE.SETTLED
			this._transplant.settledAt = Date.now()
			this.activity?.add?.( {
				kind : 'care',
				label : 'Transplant settled',
				detail : `${this._transplant.volumeL}L · new baselines adopted`,
				source : 'system',
			} )
		...
		return {
			phase : this._transplant.phase,
			volumeL : this._transplant.volumeL,
			day : status.day,
			baselines : status.settled ? 'adopted' : 'learning',
			why : status.why,
		}

	}

	/** Close it by hand, if somebody knows better than the readings do. */
	settleTransplant() {

		if ( !this._transplant ) return {
			settled : false,
			why : 'Nothing to settle.',
		}

		this._transplant.phase = TRANSPLANT_PHASE.SETTLED
		this._transplant.settledAt = Date.now()
		this._transplant.forced = true

		return {
			settled : true,
			why : 'Closed by hand. The readings had not said so yet, and somebody looking at the plant may know something they do not.',
		}
	 ...
	 *
	 * It does not see the roots. What it sees is whether this plant is behaving
	 * like one whose container is filling up — drying faster than it did, wanting
	 * water sooner — which shows months before a person notices, because a person
	 * notices once the plant is suffering.
	 *
	 * @returns {object} `{index, outlook, confidence, evidence, acts, why}`.
	 */
	rootSpace() {

		return rootSpace( this )

	}

	/**
	 * How much water this plant wants, and how long the pump should run.
	 *
	 * The archetype decides what fraction of the substrate to wet; the pot
	 * decides the volume. Both are needed — the same plant in a 12cm pot and a
	 * 40cm one wants thirty times the water, so a table of millilitres per
	 * species is wrong by an order of magnitude for one of them.
	 *
	 * @param   {object} [opts] - `{ pot, pump, season }`.
	 * @returns {object}        `{known, ml, seconds, why}`.
	 */
	wateringPlan( opts = {} ) {

		const seasonal = this.config?.hemisphere ? this.seasonalRanges( opts ) : null

		const plan = dose( {
			archetype : this.archetype,
			subgroup : this.archetype?.subgroup ?? this._subgroup,
			pot : opts.pot ?? this.config?.pot,
		}, {
			// A plant photosynthesising less drinks less, and watering a
			// houseplant in January on a July schedule is the commonest way one
			// is killed.
			season : seasonal?.applied ? 1 - ( 1 - 0.7 ) * ( seasonal.season === 'winter' ? seasonal.weight : 0 ) : 1,
		} )

		if ( !plan.known ) return plan

		const pump = pumpSeconds( plan.ml, opts.pump ?? this.config?.pump )

		return {
			...plan,
			...pump,
			known : plan.known,
			why : `${plan.why} ${pump.why}`,
		}
	 ...
	 *
	 * @param   {number} before - Soil at the moment of watering.
	 * @returns {object}        `{arrived, why}`.
	 */
	confirmWatering( before ) {

		return confirmWatering( null, before, this.memory.lastReading?.soil )

	}

	/**
	 * Go back over the record, while nothing is happening.
	 *
	 * Runs entirely on stored data — nothing is emitted and nothing acts. It runs
	 * at night because that is when nothing else wants the processor, not because
	 * darkness is needed: it would work identically at midday.
	 *
	 * @param   {object} [opts] - `{ force }`.
	...
	async consolidate( opts = {} ) {
		...
		return this.profile.time( 'consolidate', () => consolidate( this, {
			...opts,
			calibration : this.calibration,
		} ) )
	...
	/**
	 * What this plant just did, or just noticed.
	 *
	 * Listens to the event bus and diffs the states rather than being told, so
	 * something added later shows up without anybody remembering to announce it.
	 */
	get activity() {

		if ( !this._activity ) {

			this._activity = new Activity( this.config?.activity )
			this._activity.watch( this )
	...
	/** Who said what about this plant, on the strength of what. */
	get provenance() {

		this._provenance ??= new Provenance( this.config?.provenance )
		return this._provenance

	}

	/** What anything has cost. Off unless asked for. */
	get profile() {

		this._profile ??= new Profile( { enabled : this.config?.profile === true } )
		return this._profile

	}

	/**
	 * This plant's comfortable bands, bent toward the time of year.
	 *
	 * The archetype gives one set for the whole year, and it is wrong for most of
	 * it — a third of the daylight in January and a plant that has slowed to
	 * match. Needs the hemisphere, and does nothing without it rather than
	 * guessing: half the guesses would be six months wrong.
	 *
	 * Fades as this plant's own record accumulates, because after two winters its
	 * own history knows this flat better than any archetype average.
	 *
	 * @param   {object} [opts] - `{ hemisphere, at }`.
	 * @returns {object}        `{ranges, applied, season, why}`.
	 */
	seasonalRanges( opts = {} ) {

		const first = this.memory.data.readings[ 0 ]?.t
		const days = first ? ( Date.now() - new Date( first ).getTime() ) / 86_400_000 : 0

		return seasonalRanges( this.ranges, {
			hemisphere : opts.hemisphere ?? this.config?.hemisphere,
			at : opts.at,
			days,
		} )
	...
	/**
	 * Put what the system has worked out about itself into the memory file.
	 *
	 * Readings can be re-read and events re-derived. None of this can: the
	 * calibration record costs twelve overrides, which is months of somebody
	 * disagreeing with the system; the trajectory is explicitly the record that
	 * cannot be reconstructed afterwards because the code will have changed; and
	 * the adaptation log holds the original values there is no other way back to.
	 *
	 * All of it used to end with the process.
	 *
	 * @returns {Promise<object>} What was written.
	 */
	async persistSymbiont() {

		const block = {
			calibration : this.calibration
				? {
					trials : [ ...this.calibration.trials.entries() ],
					open : [ ...this.calibration.open.entries() ],
				}
				: null,
			trajectory : this._trajectory?.toJSON?.() ?? null,
			provenance : this._provenance?.toJSON?.() ?? null,
			activity : this._activity?.toJSON?.() ?? null,
			adaptation : this._adaptation?.toJSON?.() ?? null,
			at : Date.now(),
		}

		this.memory.data.symbiont = block
		await this.memory.save?.().catch( () => {} )
	...
	/**
	 * Bring it back, and say what could not be brought back.
	 *
	 * Nothing is invented. A block that was never written restores nothing and
	 * says so, because a plant resuming from an empty record and a plant that
	 * genuinely has no history should not look the same.
	 *
	 * @returns {object} `{restored, why}`.
	 */
	rehydrateSymbiont() {

		const block = this.memory?.data?.symbiont

		if ( !block || !block.at ) {

			return {
				restored : [],
				why : 'Nothing about this plant\'s own record was in the memory file. Either it has never been saved, or this is a file written before that was kept — and a plant resuming from nothing is different from one that never had anything, so this says which.',
			}
		...
		if ( block.calibration ) {
			...
			this.calibration ??= new StateCalibration()
			this.calibration.trials = new Map( block.calibration.trials ?? [] )
			this.calibration.open = new Map( block.calibration.open ?? [] )
			restored.push( 'calibration' )

		}

		if ( block.trajectory ) {

			this._trajectory = new Trajectory( block.trajectory )
			restored.push( 'trajectory' )

		}

		if ( block.activity?.entries?.length ) {

			this._activity ??= new Activity( this.config?.activity )
			this._activity.entries = block.activity.entries
			this._activity.watch( this )
			restored.push( 'activity' )

		}

		if ( block.provenance?.claims?.length ) {

			this._provenance = new Provenance( block.provenance )
			for ( const c of block.provenance.claims ) this._provenance.add( c )
			restored.push( 'provenance' )

		}

		if ( block.adaptation ) {

			this._adaptation ??= new Adaptation()
			for ( const t of block.adaptation.thresholds ?? [] ) this._adaptation.thresholds.set( t.key, t )
			restored.push( 'adaptation' )
		...
		return {
			...
			at : block.at,
			why : restored.length
				? `Resumed ${restored.join( ', ' )} from ${new Date( block.at ).toISOString()}. None of it could have been recomputed from the readings.`
				: 'The record was there and empty, which is what a plant that has never been overridden and never adapted looks like.',
		}

	}

	/**
	 * How the coupling itself has been going.
	 *
	 * The only thing here that measures the pairing rather than the plant, and
	 * the only one that can tell an electrode ageing from a plant declining —
	 * they look identical in a snapshot and completely different over a season.
	 *
	 * @param   {object} [opts] - `{ force }` to take an entry now.
	 * @returns {object}        `{known, axes, verdict, why}`.
	 */
	trajectory( opts = {} ) {

		this._trajectory ??= new Trajectory( this.config?.trajectory )
		this._trajectory.record( this, opts )

		return {
			...this._trajectory.compare(),
			log : this._trajectory,
		}

	}

	/** Everything this plant may currently be asked for, and at what size. */
	posture() {

		this.calibration ??= new StateCalibration()
		return posture( this.states(), { calibration : this.calibration } )
	...
	states() {
		...
		// Did the pot get wetter, and was anybody here when it did? A rise nobody
		// was present for is not a watering.
		const recent = readings.slice( -6 ).map( r => r.soil ).filter( Number.isFinite )
		const soilRose = recent.length >= 2 && recent.at( -1 ) - recent[ 0 ] > 8

		const log = this._presenceLog ?? []
		const since = Date.now() - 6 * 3600_000
		const presence = this.sensors?.peek?.( 'presence' )
			? {
				settled : true,
				seenSince : log.some( e => e.at >= since && e.occupancy === true ),
				disturbances : log.filter( e => e.at >= since && e.motion === true ).length,
			}
			: null
		...
		const load = stressLoad( {
			...
			disturbances : presence?.disturbances,
		} )
	...
	/**
	 * Remember what the radio said, so an electrical event can be lined up
	 * against it afterwards.
	 *
	 * @param   {object} state - From `PresenceSensor.state()`.
	 * @returns {object}       The state.
	 */
	_notePresence( state ) {

		if ( !state || state.settled !== true ) return state

		this._presenceLog ??= []
		this._presenceLog.push( {
			at : Date.now(),
			motion : state.motion,
			occupancy : state.occupancy,
		} )
		// An hour is far longer than any window that looks at it, and short
		// enough that this never becomes a memory leak on a plant left running.
		if ( this._presenceLog.length > 2000 ) this._presenceLog.shift()

		this.perception.presence = state
		return state
	...
	defense( opts = {} ) {
		...
		// Somebody brushing past produces an action potential that looks exactly
		// like the opening of a wound response, and indoors that is a commoner
		// explanation than the weather. Only asked when there is a radio to ask.
		const presence = this.sensors?.peek?.( 'presence' )
		const event = events.find( e => Number.isFinite( e.at ) )

		return defenseActivation( {
			electrode : Boolean( this.electrode || trace ),
			motion : presence
				? motionExplains( this._presenceLog ?? [], event?.at ?? Date.now() )
				: null,
			events,
	 ...
	 *
	 * Not inheritance. That moves what one plant learned to a *different* plant
	 * and fades it on the way; this is the same plant after a failed electrode, a
	 * replaced Pi or a restart, and fading its own history would discard a year
	 * of its life over a swapped cable.
	 *
	 * @returns {Promise<object>} The bundle.
	 */
	async exportIdentity() {

		const { exportIdentity } = await import( '../migration/identity.js' )
	 ...
	 * @returns {Promise<object>} `{restored, held, why}`.
	 */
	async restoreIdentity( bundle, opts = {} ) {

		const { importIdentity } = await import( '../migration/identity.js' )
	...
	async destroy() {
		...
		// Everything worked out about itself, before the process ends — which is
		// exactly when it used to be lost.
		await this.persistSymbiont().catch( () => {} )
