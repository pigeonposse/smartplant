...
describe( 'not yet is not the same as not ever', () => {

	const rows = ( n, extra = () => ( {} ) ) => Array.from( { length : n }, ( _, i ) => ( {
		t : new Date( Date.now() - ( n - i ) * 6 * 3_600_000 ).toISOString(),
		temperature : 22,
		humidity : 55,
		...extra( i ),
	} ) )

	it( 'does not send somebody out to buy a probe they already have', () => {

		const waiting = uptakeBalance( rows( 3, i => ( {
			soil : 60 - i,
			conductivity : 1200 + i,
		} ) ) )
		...
		// `missing` is printed by the diagnosis as "add this sensor".
		assert.deepEqual( waiting.missing, [] )
		assert.match( waiting.why, /Nothing to buy/ )

	} )

	it( 'still names the sensor when it genuinely is not there', () => {

		const absent = uptakeBalance( rows( 40, i => ( { soil : 60 - i * 0.4 } ) ) )

		assert.deepEqual( absent.missing, [ 'soil', 'conductivity' ] )

	} )

	it( 'works at a realistic four-readings-a-day cadence', () => {

		// The bug this replaced: a fixed twelve-hour window needed six paired
		// readings, so a plant read four times a day could never satisfy it and
		// the inference was quietly dead on most rigs.
		const r = uptakeBalance( rows( 40, i => ( {
			soil : 60 - i * 0.4,
			conductivity : 1200 + i * 3,
		} ) ) )
	...
	it( 'refuses a window that crosses a watering', () => {

		const sparse = Array.from( { length : 8 }, ( _, i ) => ( {
			t : new Date( Date.now() - ( 8 - i ) * 30 * 3_600_000 ).toISOString(),
			soil : 60 - i,
			conductivity : 1200 + i,
		} ) )

		const r = uptakeBalance( sparse )

		assert.equal( r.known, false )
		assert.equal( r.waiting, true )
		assert.match( r.why, /crosses a watering/ )

	} )

	it( 'applies the same distinction to the other two', () => {

		const fewPar = lightSaturation( rows( 4, () => ( {
			par : 200,
			co2 : 600,
		} ) ) )
		assert.equal( fewPar.waiting, true )
		assert.deepEqual( fewPar.missing, [] )

		assert.deepEqual( lightSaturation( rows( 4 ) ).missing, [ 'par', 'co2 or activity' ] )

		const fewSoilT = thermalInertia( rows( 4, () => ( { soilTemperature : 19 } ) ) )
		assert.equal( fewSoilT.waiting, true )
		assert.deepEqual( fewSoilT.missing, [] )

		assert.deepEqual( thermalInertia( rows( 4 ) ).missing, [ 'soilTemperature' ] )
