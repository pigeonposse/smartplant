...
describe( 'knowledge transfer', () => {
	...
	it( 'grades experience by time in place, not by species', async () => {
		...
		const old = await aged( bus, 'Vieja', 90 )
		const young = await aged( bus, 'Nueva', 1 )
	...
	it( 'spots the newcomer in the colony', async () => {
		...
		assert.deepEqual( found.map( f => f.peer ), [ 'nueva' ] )
	...
	it( 'teaches the room, not its own pot', async () => {
		...
		const taught = await old.colony.teach( 'nueva' )
	...
	it( 'merges several teachers into one lesson, not several', async () => {
		...
		const bus = new LoopbackBus()
		const a = await aged( bus, 'Una', 90 )
		const b = await aged( bus, 'Otra', 80 )
		const c = await aged( bus, 'Tercera', 70 )
		const young = await aged( bus, 'Nueva', 1 )
	...
	it( 'says so when nobody in the colony knows the room either', async () => {
		...
		const bus = new LoopbackBus()
		const a = await aged( bus, 'Una', 1 )
		const b = await aged( bus, 'Otra', 1 )
	...
	it( 'refuses to merge across species', () => {
