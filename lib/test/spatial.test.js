...
import { stressLoad } from '../src/states/index.js'
import {
...
describe( 'presence, and what a radio can honestly say', () => {
	...
	it( 'says plainly what it cannot see', async () => {
		...
		// Deterministic: a room that occasionally reads occupied turns an
		// assertion about the quiet case into a coin toss.
		let tick = 0

		const s = new PresenceSensor( {
			sensing : SENSING.RSSI,
			sample : async () => -50 + ( tick++ % 4 ) * 0.1,
			settleSamples : 20,
		} )
		await s.connect()
		await feed( s, 30 )
...
describe( 'what the panel shows, and what the states make of it', () => {

	const ring3 = ( fn, n = 360 ) => ( {
		ranges : Array.from( { length : n }, ( _, i ) => fn( i ) ),
		angleMin : 0,
		angleMax : 2 * Math.PI,
	} )
	...
	const equipped = async () => {
		...
		const lidar = await p.attachSensor( {
			driver : 'lidar',
			scan : async () => ring3( i => ( i > 85 && i < 95 ? 0.31 : 3 ) ),
		} )
		await lidar.read()

		// Deterministic rather than random: a room that sometimes reads occupied
		// makes every assertion below a coin toss, and a flaky test about a
		// negative control is worse than none.
		let tick = 0
		await p.attachSensor( {
			driver : 'presence',
			sensing : SENSING.RSSI,
			settleSamples : 5,
			sample : async () => -50 + ( tick++ % 3 ) * 0.1,
		} )
		for ( let i = 0; i < 12; i++ ) await p.read()

		return p

	}

	it( 'puts the space in the inventory, not among the vitals', async () => {

		const p = await equipped()
		const { snapshot } = await import( '../src/dashboard/index.js' )
		const snap = await snapshot( p )

		// A scan is not a number that goes up and down, so it does not belong
		// beside temperature.
		assert.ok( !snap.vitals.some( v => v.metric === 'range' ) )

		assert.equal( snap.space.returns, 360 )
		assert.deepEqual( snap.space.neighbours, [ {
			id : 'willow',
			metres : 0.31,
		} ] )
		assert.equal( snap.presence.sensing, 'rssi' )
		assert.equal( snap.layers.lidar, true )

		await p.destroy()

	} )

	it( 'says the scan is too old rather than showing an old one', async () => {

		const p = await equipped()
		p.sensors.peek( 'lidar' ).staleAfter = 1
		await new Promise( r => setTimeout( r, 20 ) )

		const { snapshot } = await import( '../src/dashboard/index.js' )
		const snap = await snapshot( p )

		assert.equal( snap.space.stale, true )
		assert.match( snap.space.why, /moved a chair through/ )

		await p.destroy()

	} )

	it( 'refuses to record an unattended rise as a watering', async () => {

		const p = await equipped()

		for ( const soil of [ 30, 32, 35, 40, 48, 55 ] ) {
		...
		const water = p.states().water_stress_internal

		// Somebody watered it is the only explanation that requires somebody to
		// have been there. A leak, rain through a window and a probe drifting up
		// all want looking at, and none wants the schedule adjusted.
		assert.equal( water.pattern, 'unattended-rise' )
		assert.equal( water.acts, true )
		assert.equal( ( await p.water() ).refused, true )

		await p.destroy()

	} )

	it( 'counts being handled as load, lightly', () => {

		const quiet = stressLoad( {
			events : { ratePerHour : 3 },
			episodes : 2,
		} )
		const busy = stressLoad( {
			events : { ratePerHour : 3 },
			episodes : 2,
			disturbances : 40,
		} )

		assert.ok( busy.evidence.some( e => e.signal === 'handled-often' ) )
		assert.ok( !quiet.evidence.some( e => e.signal === 'handled-often' ) )

		// A nuisance rather than an injury: it counts, and on its own it does not
		// carry a plant from quiet to high.
		assert.equal( quiet.level, 'medium' )
		assert.equal( busy.level, 'medium' )

		// It does tip one that was already close.
		const nearly = stressLoad( {
			events : { ratePerHour : 8 },
			episodes : 2,
		} )
		const tipped = stressLoad( {
			events : { ratePerHour : 8 },
			episodes : 2,
			disturbances : 40,
		} )
		assert.equal( nearly.level, 'medium' )
		assert.equal( tipped.level, 'high' )

	} )

	it( 'checks the delivery layer, the reasoner, safety and voice', async () => {

		const p = await equipped()
		const areas = ( await p.systemDiagnosis( { probeAI : false } ) ).checks.map( c => c.area )

		for ( const a of [ 'delivery', 'knowledge', 'safety', 'voice', 'spatial', 'presence' ] ) {
	...
	it( 'warns about a colony with one link and nothing behind it', async () => {

		const p = await equipped()
		await p.joinColony( { transport : new LoopbackBus().endpoint( 'full' ) } )

		const delivery = ( await p.systemDiagnosis( { probeAI : false } ) ).checks
			.find( c => c.area === 'delivery' )

		assert.equal( delivery.result, 'warn' )
		assert.match( delivery.fix, /One transport is one point of failure/ )
