...
describe( 'one door, and what may never be shut', () => {
	...
	it( 'is the same rule everywhere, which it was not before', async () => {

		// The probe used to check `posture.hold`, which reads the level and
		// ignores the confidence — so a low-confidence defence estimate blocked
		// a probe while being explicitly not allowed to gate anything else.
		// Going through the one door made that inconsistency impossible.
		...
		// One reading, so the environment could never be checked and the estimate
		// is low-confidence however alarming its level.
		assert.equal( plant.states().defense_activation.confidence, 'low' )
		assert.equal( plant.states().defense_activation.acts, false )
		assert.equal( plant.mayI( ACTION.PROBE.id ).allowed, true )
