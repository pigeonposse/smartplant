...
describe( 'colony over a real socket', () => {
	...
	it( 'carries the whole colony across sockets', async () => {
		...
		const rosa = await plant( 'Rosa', 90 )
		const lila = await plant( 'Lila', 80 )
		const nueva = await plant( 'Nueva', 1 )

		const server = new ColonyServer( { id : 'rosa' } )
		await rosa.joinColony( { transport : server } )
		await lila.joinColony( { transport : new ColonyClient( {
			id : 'lila',
			port : server.port,
		} ) } )
		await nueva.joinColony( { transport : new ColonyClient( {
			id : 'nueva',
			port : server.port,
		} ) } )
		await settle()
		...
		// silently do nothing.
		assert.deepEqual( ( await rosa.colony.peers() ).sort(), [ 'lila', 'nueva' ] )
		assert.deepEqual( ( await lila.colony.peers() ).sort(), [ 'nueva', 'rosa' ] )

		assert.equal( ( await rosa.colony.ask( 'lila', 'sense.vpd-perception' ) ).ok, true )
		assert.equal( ( await lila.colony.ask( 'rosa', 'consensus.identity' ) ).data.name, 'Rosa' )
		// Neither client has a socket to the other; the listener relays.
		assert.equal( ( await lila.colony.ask( 'nueva', 'consensus.presence' ) ).ok, true )

		const all = await nueva.colony.askAll( 'sense.vpd-perception' )
		assert.equal( all.answers.length, 2 )

		const refused = await rosa.colony.ask( 'lila', 'rhizo.mycelium-connect' )
	...
	it( 'teaches a newcomer across the wire', async () => {
		...
		const old = await plant( 'Vieja', 90 )
		const young = await plant( 'Joven', 1 )

		const server = new ColonyServer( { id : 'vieja' } )
