...
describe( 'what it changes', () => {
	...
	it( 'refuses a spectral probe on a plant that is defending itself', async () => {
		...
		// Enough history for the negative control to run. Without it the defence
		// estimate is low-confidence, and a low-confidence state does not gate
		// anything — which is the doctrine, and is now applied consistently
		// because the probe goes through the same door as everything else.
		for ( let i = 0; i < 12; i++ ) {
		...
		assert.equal( refused.blockedBy[ 0 ].state, 'defense_activation' )
