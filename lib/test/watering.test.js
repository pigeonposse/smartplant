...
describe( 'what just happened', () => {
	...
	it( 'counts a repeated line instead of repeating it', () => {
		...
		// A routine inspection finding the same thing a moment later has not
	...
	it( 'keeps advancing while something stays true', () => {

		const a = new Activity()
		const t0 = Date.parse( '2026-08-15T17:33:00Z' )

		// The failure this replaced: a line arriving faster than the dedupe
		// window pushed the window out on every repeat, so it never aged out.
		// Half an hour of "Thirsty" collapsed into one entry stamped 17:33 —
		// with the clock reading 17:59 and the feed insisting it was current.
		for ( let s = 0; s < 27 * 60; s += 3 ) a.add( {
			kind : 'state',
			label : 'Thirsty',
			at : t0 + s * 1000,
		} )

		assert.equal( a.entries.length, 27, 'twenty-seven minutes must be twenty-seven lines' )

		const newest = a.recent( { limit : 1 } )[ 0 ]
		assert.equal( newest.at, t0 + 26 * 60_000 )
		// And the count is now "within this minute", which is why the feed no
		// longer needs a second clock to say when it was last true.
		assert.equal( newest.repeated, 20 )

	} )

	it( 'still folds a line that recurs slowly into one entry per minute', () => {

		const a = new Activity()
		const t0 = Date.parse( '2026-08-15T17:33:00Z' )

		for ( let m = 0; m < 5; m++ ) a.add( {
			kind : 'system',
			label : 'Component degraded',
			at : t0 + m * 60_000,
		} )

		assert.equal( a.entries.length, 5 )
		assert.ok( a.entries.every( e => ( e.repeated ?? 1 ) === 1 ) )
