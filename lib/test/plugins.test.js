/**
 * Integration test across all eighteen official plugins.
 *
...
describe( 'official plugins', () => {
	...
	// The other eight do not answer with `advice`, so they are not in the table
	// above — but every one of them still has to install on a real kernel
	// alongside the rest, which is the failure this whole file exists to catch.
	it( 'all eighteen coexist, including the ones that answer with measurements', async () => {
		...
		// Colony is the one that refuses to install without somewhere to talk.
		await plant.use( ( await import( '@smartplant/colony' ) ).default, { transport : bus.endpoint( 'ivy' ) } )

		assert.equal( plant.plugins.size, 18 )
		await plant.read()

		// And each of the new five answers without hardware, by refusing.
		assert.equal( plant.plugin( 'transplant' ).space().index, 'unknown' )
		assert.equal( ( await plant.plugin( 'thermal' ).stress() ).known, false )
		assert.equal( plant.plugin( 'presence' ).uvbAllowed().allowed, false )
		assert.equal( ( await plant.plugin( 'season' ).now() ).known, false )
		assert.equal( plant.plugin( 'energy' ).forecast().unlimited, true )
