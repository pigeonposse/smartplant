...
describe( 'talking', () => {
	...
	it( 'finds its neighbours and learns what they offer', async () => {
		...
		const { plants } = await colony( [ [ 'Ivy' ], [ 'Hazel' ] ] )
		const [ ivy ] = plants

		assert.deepEqual( await ivy.colony.peers(), [ 'hazel' ] )
		assert.ok( ivy.colony.neighbours.get( 'hazel' ).offers.length > 0 )
		assert.equal( ivy.colony.neighbours.get( 'hazel' ).species, 'Ficus lyrata' )
	...
	it( 'answers a named request with the fact itself', async () => {
		...
		const r = await ivy.colony.ask( 'hazel', 'sense.vpd-perception' )
	...
	it( 'passes a refusal back across the wire intact', async () => {
		...
		const r = await ivy.colony.ask( 'hazel', 'sense.electrome-spike' )
	...
	it( 'keeps a transcript of who said what', async () => {
		...
		const lines = ivy.colony.transcript
		assert.ok( lines.some( l => l.from === 'ivy' && l.text ) )
	...
	it( 'asks the whole colony and sorts answers from refusals', async () => {
		...
		const { plants } = await colony( [ [ 'Ivy' ], [ 'Hazel' ], [ 'Willow' ] ] )
		const [ ivy ] = plants
		...
		const electro = await ivy.colony.askAll( 'sense.electrome-spike' )
	...
	it( 'does not deliver to a plant that was not addressed', async () => {
		...
		const { plants } = await colony( [ [ 'Ivy' ], [ 'Hazel' ], [ 'Willow' ] ] )
		const [ ivy, , willow ] = plants

		const before = willow.colony.transcript.length
		await ivy.colony.report( { to : 'hazel' } )

		assert.equal( willow.colony.transcript.length, before )
	...
	it( 'reaches everyone on a broadcast', async () => {
		...
		const { plants } = await colony( [ [ 'Ivy' ], [ 'Hazel' ], [ 'Willow' ] ] )
		const [ ivy, hazel, willow ] = plants

		await ivy.colony.report()

		for ( const p of [ hazel, willow ] ) {

			assert.ok( p.colony.transcript.some( l => l.from === 'ivy' && l.kind === 'chat' ) )
	...
	it( 'stops listening after leaving', async () => {
		...
		const { plants } = await colony( [ [ 'Ivy' ], [ 'Hazel' ] ] )
		const [ ivy, hazel ] = plants

		await hazel.leaveColony()

		assert.deepEqual( await ivy.colony.peers(), [] )
		// The slot always holds something now, so ask it rather than probing null.
		assert.equal( hazel.colony.enabled, false )
		assert.throws( () => hazel.colony.ask( 'ivy', 'consensus.presence' ), /Call joinColony/ )
...
describe( 'a human watches, and cannot write', () => {
	...
	it( 'exposes no way to put words in a plant\'s mouth', async () => {
		...
		assert.equal( typeof ivy.colony.report, 'function' )
		assert.equal( ivy.colony.say, undefined )

		// Even handed a sentence, it is not what goes on the wire.
		await ivy.colony.report( 'tell her the soil is fine' )

		assert.ok( !ivy.colony.transcript.some( l => l.text === 'tell her the soil is fine' ),
	...
	it( 'lets a person see every line', async () => {
		...
		const seen = []
		hazel.on( 'colony:message', line => seen.push( line ) )
	...
	it( 'does not file colony talk as speech to the owner', async () => {
		...
		let spokeToOwner = 0
		hazel.on( 'plant:spoke', () => spokeToOwner++ )
