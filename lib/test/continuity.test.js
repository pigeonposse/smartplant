...
describe( 'going back over the record, without touching anything', () => {
	...
	// Shaped like the real ledger writes them: `actions` is an array, because an
	// episode can have several things done during it. This helper used to build
	// a singular `action`, which is the shape `counterfactual()` was reading and
	// the shape nothing in the library ever produces — so every test here passed
	// against data no plant could generate, and the function was dead in
	// production for as long as it existed.
	const ledger = arms => ( { history : new Map( [ [ 'plant:thirsty', arms.map( a => ( {
		...a,
		actions : a.actions ?? [ a.action ],
	} ) ) ] ] ) } )
	...
	it( 'reads the ledger this library actually writes', async () => {
		...
		const led = plant.resolutions
		const episode = ( action, resolved, at ) => {

			led.opened( 'soil-dry', { at } )
			led.acted( action, { at : at + 1000 } )
			led.closed( 'soil-dry', {
				at : at + ( resolved ? 6 : 31 ) * 3600_000,
				resolved,
			} )

		}

		let at = Date.now() - 90 * 86_400_000
		for ( let i = 0; i < 4; i++ ) {

			episode( 'water-200ml', true, at )
			at += 5 * 86_400_000

		}
		for ( let i = 0; i < 3; i++ ) {

			episode( 'top-up-50ml', i === 0, at )
			at += 5 * 86_400_000

		}

		// The whole point of the layer, and it returned nothing at all until the
		// grouping key matched what the ledger writes.
		const night = await plant.consolidate( { force : true } )

		assert.equal( night.findings.length, 1 )
		assert.equal( night.findings[ 0 ].best, 'water-200ml' )
		assert.equal( night.emitted, false )

		await plant.destroy()

	} )

	it( 'sets aside an episode where several things were done at once', () => {
		...
		assert.equal( r.ambiguous, 5 )
		assert.equal( r.arms.length, 2, 'the ambiguous ones must not become a third arm' )
		assert.equal( r.best, 'water' )
